...
const LAW_PATH = 'laws/regulation/nl/ministeriele_regeling/regeling_bekostiging_wpo_en_wec/2025-01-01.yaml';
...
describe('een zelf bewaarde variant', () => {
  ...
  it('levert een wet op die de engine accepteert en die van de basis afwijkt', async () => {
    ...
    const basisYaml = readFileSync(resolve(lawsDir, 'regulation/nl/ministeriele_regeling/regeling_bekostiging_wpo_en_wec/2025-01-01.yaml'), 'utf-8');

    // Dezelfde bewerking die het parameterpaneel maakt: één waarde in de YAML.
    const bewerkt = patchDefinitionValue(basisYaml, '34', 'kwartalen_eerste_jaar', 6);
    ...
    expect(tekst).toMatch(/kwartalen_eerste_jaar[\s\S]{0,120}?6/);
