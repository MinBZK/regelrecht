...
impl Cell {
    ...
    fn execute(
        ...
        let legal_character = produces.and_then(|produces| produces.legal_character.clone());
        ...
        let outputs: BTreeMap<String, Value> = definition
            ...
            .collect();
        // Ketste het besluit af op een voorwaarde? Dan is het een **afwijzing**:
        // hetzelfde gram, hetzelfde rechtskarakter, maar een ander besluittype en
        // straks geen verplichtingen (Awb 1:3 lid 2 — een beschikking omvat ook
        // de afwijzing van de aanvraag). Het artikel bij elke grond komt van
        // dezelfde resolver op hetzelfde moment als het rechtskarakter hierboven:
        // wat de wet zegt, zegt ze in de versie die op dit moment gold.
        let afwijzingsgronden = definition.afwijzingsgronden(&outputs, |output| {
            resolver
