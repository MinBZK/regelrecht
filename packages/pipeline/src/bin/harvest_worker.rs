use regelrecht_pipeline::config::WorkerConfig;
use regelrecht_pipeline::health::spawn_health_server;
use regelrecht_pipeline::worker::run_harvest_worker;

#[tokio::main]
async fn main() {
    regelrecht_shared::telemetry::init_subscriber("info");

    let config = match WorkerConfig::from_env() {
        Ok(config) => config,
        Err(e) => {
            tracing::error!(error = %e, "failed to load configuration");
            std::process::exit(1);
        }
    };

    // Bind the health endpoint before starting the worker — failure is fatal
    // because RIG requires the liveprobe to be reachable; without it the pod
    // restart-loops.
    let health_handle = match spawn_health_server(None).await {
        Ok(h) => h,
        Err(e) => {
            tracing::error!(error = %e, "failed to bind health endpoint");
            std::process::exit(1);
        }
    };

    tokio::select! {
        result = run_harvest_worker(config) => {
            if let Err(e) = result {
                tracing::error!(error = %e, "harvest worker exited with error");
                std::process::exit(1);
            }
        }
        result = health_handle => {
            match result {
                Ok(_) => tracing::error!("health endpoint task exited unexpectedly"),
                Err(e) => tracing::error!(error = %e, "health endpoint task panicked"),
            }
            std::process::exit(1);
        }
    }
}
