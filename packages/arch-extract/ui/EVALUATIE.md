...
`GET /api/model` (2422 nodes, 1541 relaties), gebruiken dezelfde rollup
...
|---|---|---|---|
| `container` | 24 | 1541/1541 | **1389** |
| `component` | 823 | 1541/1541 | 0 |
| `code` | 2422 | 1541/1541 | 0 |

De eerste rij is meteen de belangrijkste bevinding van het hele ticket: **90%
van alle relaties (1389 van 1541) zit binnen één crate/app.** Op containerniveau
is er dus maar 152 relaties aan echte lijnen te tekenen. Elke techniek die
alleen containerniveau laat zien, laat 90% van het verkeer als getal achter.
...
DOM: 2422 Vue Flow-nodes halen dit niet, 2422 `fillRect`-aanroepen wel.
...
run over 823 componenten geeft een correcte maar onbruikbare sliert, omdat de
...
*Map op containerniveau: 24 knopen, 152 lijnen. `corpus` (78) en `editor-api` (59) zijn de dikste blokken.*

![Map, componentniveau](evaluatie/map-component.jpg)
*Map op componentniveau (1,7×): wijken met interne structuur. `AppState`, `AccountRecord`, `corpus_handlers`, `EditorView.vue` en `TrajectCorpus` krijgen automatisch een label omdat ze bij de tien grootste horen.*
...
gedrukt worden. `corpus`, `editor-api` en `github` liggen op containerniveau
naast elkaar met dikke lijnen ertussen; dat leest als één subsysteem.

![Map, codeniveau](evaluatie/map-code.jpg)
*Map op codeniveau (6,3×): de sliert komt terug binnen de grote wijken. `engine` en `pipeline` hebben elk ~400 code-nodes in weinig rangen; je kijkt naar een kolom.*

**Op codeniveau valt Map om.** De tweetraps-truc helpt niet meer als één wijk
zelf 400 knopen heeft: binnen `engine` is de aspectverhouding weer 1:5 en zie je
...
*Radiaal op componentniveau (1,7×): 823 stippen op de ring. De dikke touwen tussen `corpus`, `editor-api` en `engine` zijn er meteen uit; `AppState`, `AccountRecord` en `corpus_handlers` zijn de dikste stippen.*

**Wie is een hub.** Ja, maar minder direct dan bij Map. `AppState` en
`AccountRecord` zijn duidelijk dikkere stippen dan hun buren, en op de ring liggen
ze naast elkaar. Waar het beeld beter in is dan Map: je ziet *waar op de ring*
de dikte zit — de hele rechterhelft (`corpus`, `editor-api`, `engine`) is dik en
...
hebben geen pijlpunten (die zouden bij 1175 curves alleen maar ruis zijn). Je
...
blijft binnen. Je leest in één oogopslag af dat `frontend`↔`frontend-shared` een
...
*Radiaal op codeniveau (6,3×, uitgepand naar de rand): 2422 stippen. De ring is dan 6× het venster, dus je kunt hem alleen nog stukje bij beetje aflopen.*
...
het codeniveau (2422² ≈ 5,9 miljoen cellen) net zoveel als de rest.
...
*Matrix op componentniveau (1,7×): 823×823 met 1175 gevulde cellen. De klontering op de diagonaal is echt; de gefragmenteerde kleurstrip links is dat ook.*
...
een volle rij van 139 cellen tussen 823 rijen niet zomaar aanwijzen, omdat één
...
weten *wat* die kolom is moet je de as terug omhoog volgen. Bij 823 rijen is dat
...
dezelfde as. Op componentniveau ligt **571 van de 1175** relaties onder de
diagonaal; dat zijn geen 571 cykels, dat is ruis van een ordening die op
...
containers uiteen in **181 aaneengesloten stukken** langs de as: de
...
*Matrix op codeniveau (6,3×): 2422×2422 met 1541 gevulde cellen — 0,03% dichtheid. Je kijkt naar een sterrenhemel.*

**Op codeniveau wordt de matrix te leeg.** 1541 cellen in 5,9 miljoen posities
...
2. **Clusters die de mappenstructuur tegenspreken.** Matrix is hier duidelijk
   ...
   wordt. De 181-stukken-bevinding uit de Matrix zou Map nooit hebben opgeleverd.
