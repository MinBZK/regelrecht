...
//!
//! Endpoints (all require an authenticated editor session):
...
const SESSION_KEY_STATE: &str = "github_oauth_state";
...
pub async fn login(
    ...
    // 122-bit random, single-use CSRF state.
    ...
    session_insert(&session, SESSION_KEY_STATE, csrf_state).await?;
...
pub async fn callback(
    ...
    let stored_state: Option<String> = session.get(SESSION_KEY_STATE).await.ok().flatten();
    ...
    let (code, req_state, stored_state) = match (params.code, params.state, stored_state) {
        (Some(code), Some(req_state), Some(stored_state)) => (code, req_state, stored_state),
        _ => {
    ...
    let matches: bool = req_state.as_bytes().ct_eq(stored_state.as_bytes()).into();
    if !matches {
