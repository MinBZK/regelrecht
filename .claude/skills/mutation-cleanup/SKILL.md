...
Working from a failing pull-request gate instead? The gate runs in four
shards, and each leaves its files in its own artifact,
`mutation-report-diff-shard-0` to `-3`. Only a red shard has something to
fix; the list is short by construction.
