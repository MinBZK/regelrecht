# RFC-013: Execution Provenance — Reproducible Law Execution Across Versions and Organisations
...
- Execution Receipts satisfy Awb Art. 3:46 (motivation), AERIUS (auditability), and EU AI Act Art. 12 (logging). The receipt, including the full scope of loaded regulations, is the archivable artifact that proves how a computation was performed.
- Independent versioning lets other organisations build their own engine on their own release cadence. A future conformance test suite (separate RFC) will formalize the behavioral contract.
...
- Versioning the inter-engine protocol independently from the engine lets organisations upgrade at their own pace.
- Execution Receipts are additive. They extend `ArticleResult` without changing execution logic. Schema enforcement and CI checks can be rolled out phase by phase.
- The engine declares which schemas it supports. Adding a new schema version does not break old regulations. Dropping support for an old schema is an explicit, versioned decision.
