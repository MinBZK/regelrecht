...
The engine's operation set is deliberately small — arithmetic, comparison, logical, conditional, and a handful of date operations. It only grows when real legal texts demand a new construct.
...
An optional `untranslatables` field on the `machine_readable` section:
...
When the engine encounters articles with `untranslatables`, behavior is controlled by the `--untranslatable` flag:

| Mode | Behavior | Use case |
|------|----------|----------|
| `error` (default) | Hard error, stop execution | CI, production |
| `propagate` | Produce `UNTRANSLATABLE` value that taints downstream outputs | Audit, analysis |
| `warn` | Skip article, log warning in trace | Development, exploration |
| `ignore` | Execute anyway | Human-verified workarounds |
...
- **Fail-fast default** ensures gaps are never silently ignored in CI or production
- **Propagation mode** gives full visibility into which outputs are affected without stopping execution
- **Drives the engine roadmap** — untranslatables tell us which operations to add next, based on real legal texts rather than speculation
