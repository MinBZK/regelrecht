# RFC-015: Engine Policy — Externalizing Domain Knowledge as Layered Configuration
...
The engine's founding principle is **zero built-in domain knowledge**: all legal and domain knowledge comes from law YAML files and parameters. The engine provides pure operations (arithmetic, comparison, date math) and a resolution framework. It does not know about Easter, King's Day, public holidays, tax rates, or government structure. RFC-007 established this principle; RFC-008 reinforced it by making Easter an external parameter.

In practice, the engine violates this principle in four places:

| What | Where | Domain knowledge |
|------|-------|-----------------|
| Eurocent rounding | `service.rs` | The string `"eurocent"` triggers float-to-integer rounding at the output boundary. The engine knows that eurocent values must be whole numbers. |
| Territorial scoping | `resolver.rs` | The field name `gemeente_code` is hardcoded as the scope dimension for territorial filtering. The engine knows about Dutch municipal codes. |
| Regulatory layer hierarchy | `priority.rs` | 12 layers (Verdrag through Uitvoeringsbeleid) with hardcoded rank ordering for lex superior resolution. The engine knows the Dutch constitutional hierarchy. |
| Delegation type validation | `service.rs` | When an open term declares `delegation_type`, the engine validates that the implementing regulation's `regulatory_layer` matches. The engine knows which layers can delegate to which. |

These are not bugs — the engine needs this knowledge to function. But embedding it in Rust source code means:

- Changing the hierarchy requires a code change and engine release
- Organizations cannot adapt the engine to their context without forking
- The knowledge is invisible to audit (not in the Execution Receipt)
- It contradicts the stated design principle

The knowledge exists because no law or regulation defines it explicitly. The regulatory layer hierarchy is constitutional doctrine, not a statute article. Eurocent rounding is financial convention, not a legal rule. These are the rules *about* law execution that no specific law declares.

## Decision

### 1. Engine Policy as structured configuration

Domain knowledge that the engine needs but that does not come from any specific law is externalized into an **Engine Policy** file: a YAML configuration file that the engine reads at startup.

The Engine Policy is not part of the corpus. It is engine configuration, expressed in a structured YAML format that can be versioned and audited, instead of being compiled into the binary.
...
Date arithmetic (RFC-007) stays in the engine. The clamping rule (Jan 31 + 1 month = Feb 28) is standard calendar math confirmed by the Hoge Raad (HR 1 September 2017, ECLI:NL:HR:2017:2225). Age calculation stays. Day-of-week numbering stays (ISO 8601). These are not domain knowledge — they are the same in any jurisdiction.

## Why

### Benefits

The engine binary contains no Dutch legal knowledge after this change. Everything comes from law YAML or policy YAML, restoring the zero-domain-knowledge principle.

The policy lands in the Execution Receipt, so an auditor can see which hierarchy and scoping rules were active for any decision. Policy changes are tracked in version control and reviewed in PRs like any other configuration.

Organizations can customize without forking. A water board adds `waterschap_code` as a scope dimension. A test harness uses a simplified hierarchy. A future non-Dutch deployment replaces the entire policy. Adding `provincie_code` becomes a YAML change, not a code change.
...
- Rejected: the corpus contains real law. Engine operating rules are not law — they are meta-rules about how to process law. Mixing them creates confusion about what is legally authoritative.
