...
> 💡 **Reminder facilitator**: benoem hier het onderscheid **factual vs judgment untranslatable**:
> - **Factual** (feitelijk vaststelbaar door systeem): bullet 12 *nadere voorwaarden niet nagekomen* · bullet 13 *gemeente vergoedt aanslag*
> - **Judgment** (oordeelsvorming / prognose): eindblok 1 *wisselende inkomens verwacht* · eindblok 2 *verbetering financieel verwacht*
>
> Factual kán als caller-parameter in de OR-gate. Judgment niet zonder waarde-verlies. Parkeer beslissing naar B3 in Deel 5.
...
| B3b | **4 untranslatable 26.1.9-gronden**: per grond factual (gate-waardig) of judgment (blijft untranslatable)? | Alle 4 blijven nu untranslatable          | A caller-param / B twee-traps / C open-checks-lijst / D workflow (huidig) | [ ] |

### B3b — toelichting (untranslatable-keuze)

> 💡 **Reminder**: loop per grond langs of hij **factual** of **judgment** is:
>
> | Grond | Type | Gate-waardig? |
> |---|---|---|
> | Bullet 12 *nadere voorwaarden niet nagekomen* | Factual | Ja — HHNK-systeem kent voorwaarde-status |
> | Bullet 13 *gem. sociale dienst vergoedt* | Factual | Ja — gemeente-systeem weet dit |
> | Eindblok 1 *wisselende inkomens verwacht* | Judgment | Nee — prognose |
> | Eindblok 2 *verbetering financieel verwacht* | Judgment | Nee — prognose |
>
> **4 opties** per grond:
> - **A** Caller-parameter: ontvanger-systeem stelt boolean → in OR-gate (alleen factual)
> - **B** Twee-traps: engine geeft *kan_mogelijk*, ontvanger zet *ontvanger_akkoord*, definitief = AND
> - **C** Open-checks-lijst: engine produceert checklist voor ontvanger, blijft mechanisch agnostisch
> - **D** Workflow (huidig): buiten engine, ontvanger-procedure
...
| B8  | **Auto-drempel €3 350**: grondslag voor HHNK-doorwerking             | Rijks-Leidraad-2008 overrided URI art 12 blind | rijks-override / HHNK-eigen-override / beide | [ ]   |

### B8 — toelichting (cruciaal juridisch punt)

Leidraad Invordering 2008 geldt formeel voor **rijksbelastingen**. Waterschapsbelastingen vallen daar niet onder. HHNK krijgt URI 1990 via **Regeling kwijtschelding belastingen medeoverheden** (RKBM), maar die verwijst alleen naar URI (ministeriële regeling), niet naar rijks-beleidsregels.

HHNK-leidraad 2026 aanhef verklaart beleidsmatig *"aansluiting gezocht bij de Leidraad Invordering 2008"* — dat is de HHNK-grondslag voor de €3 350.

**Huidige YAML**: Leidraad 2008 overrided URI art 12's `auto_als_vermogen` blind — werkt ook voor HHNK. Juist in uitkomst, maar de grondslag-attributie in de YAML klopt niet: HHNK gebruikt €3 350 niet via directe rijks-doorwerking maar via HHNK's eigen beleidsmatige aansluiting.

**Drie opties**:
- `rijks-override` = huidige situatie laten (engine werkt, juridisch grondslag via HHNK-aanhef geclaimed)
- `HHNK-eigen-override` = HHNK-leidraad krijgt eigen override op URI art 12 (naast rijks-override), met expliciete legal_basis naar HHNK-aanhef
- `beide` = rijks-override voor rijksbelastingen, HHNK-eigen voor waterschapsbelastingen (scheidt scopes)

Tekenend voorbeeld van: **AI-vertaling nam kortere route dan juridisch houdbaar**. Experts moeten kiezen.
