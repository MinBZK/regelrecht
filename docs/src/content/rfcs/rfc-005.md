...
We call this feature **notes** (NL: *notities*), not "annotations". In Dutch
case-law practice an *annotatie* (or *noot*, "m.nt.") is an established genre: a
scholarly commentary written by a jurist on a court ruling, published in a
jurisprudence journal. Using "annotation" for arbitrary text comments looked like a
clash with that meaning. The word is not reserved for that genre, though. In the
STOP/TPOD standard for the Omgevingswet an *annotatie* is metadata added to the text
of a regulation (see also *Wendbaar wetgeven*, p. 29), and a legal review of this
RFC recommended using the word for that reason: legal and technical terminology
would then line up. Whether to switch is an open question, recorded at the end of
this RFC.
...
by external parties ("bring your own notes"). The format is storage-agnostic
by design.
...
`prefix` and `suffix` carry no meaning of their own. They are the text that happens to stand directly before and after the quote in the document, and they serve only to find the right occurrence of `exact` when the same words appear more than once. That ". Voor een verzekerde" opens a sentence about something else is the point: it is where the quoted passage ends.
...
2. **Optional hint** - Add `regelrecht:hint` with a W3C selector as recorded
   provenance: which article and position the note was written on. The hint is
   non-authoritative and does not narrow the search. A hinted shortcut could
   hide a competing occurrence elsewhere in the law, silently resolving a
   genuinely ambiguous selector onto a possibly renumbered article, so
   resolution always scans the whole law (see
   [RFC-018](/rfcs/rfc-018) for the rationale).
   ...
   This records: "this note was written on article 2 at position 45-56". The
   resolver ignores it; the note is found by its text alone.
...
4. **No changes to legal text**: Notes are completely separate from the source text
...
- Article numbers serve only as recorded, non-authoritative hints
...
`schema/v0.5.3/annotation-schema.json`, which additionally defines `creator`,
...
properties:
  ...
  regelrecht:hint:
    type: object
    description: Optional recorded hint (non-authoritative, not used in resolution)
    properties:
      type:
        const: CssSelector
      value:
...
- **Call the feature "annotaties" after all?** Mariette Lokin ([#1094](https://github.com/MinBZK/regelrecht/pull/1094)) recommends it: *annotatie* is used well beyond commentary on court rulings, and STOP/TPOD uses it for metadata on regulation texts. Switching would touch the editor, the corpus directory `annotations/` (already named that way) and RFC-018.
...
- **2026-09-25.** The terminology section called *annotatie* a protected genre reserved for commentary on court rulings. It is not: STOP/TPOD uses the word for metadata added to regulation texts. The decision to call the feature "notes" stands; the claim that it had to be so was corrected. Example 2 now explains what `prefix` and `suffix` are for. Source: review by Mariette Lokin, [#1094](https://github.com/MinBZK/regelrecht/pull/1094), and a team member's review comment on it.
