...
with that established meaning. [Mariette: annotatie wordt veel breder gebruikt, het is niet een 'beschermd genre' voor noten bij arresten; zie ook p. 29 van Wendbaar wetgeven; in de STOP/TPOD standaard voor de omgevingswetgeving is het specifiek gebruikt voor toevoegen van metadata aan regelingsteksten. Je zou het prima kunnen gebruiken, mooi is dat dan de juridische en 'technische' terminologie ook op elkaar aansluiten.] 
...
target:
  ...
  selector:
    ...
    prefix: "heeft de verzekerde aanspraak op een "
    suffix: ". Voor een verzekerde" [Mariette: waarom is dit de suffix? Het hoort bij normpremie, niet bij zorgtoeslag toch?]
body:
