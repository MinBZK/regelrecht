...
When a completed enrichment is committed, the worker (gated by an opt-in flag)
resolves each declared entry to a BWB id and enqueues a follow-up harvest.
...
The loop is designed to be safe to leave on:

- **Opt-in**: off by default, enabled via `HARVEST_RELATED_LEGISLATION=true`.
...
- **Unambiguous-but-loose SRU match.** A single SRU title hit is accepted as-is;
  ...
  enqueue the wrong law. Impact is low — the loop is opt-in, harvest-only, and its
  output lands on a reviewable `enrich/{provider}` branch — but it is why `>1`
...
client-taking function shared with the API handler, the worker hook with hybrid
resolution and summary logging, the two config flags, and the enrichment-skill
step that writes the envelope. Not yet built: a UI/admin surface for the
