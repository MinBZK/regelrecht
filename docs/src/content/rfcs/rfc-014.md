...
> **Implementation note.** The core of this RFC exists: conformance manifests (`conformance/v0.5.0/`, `v0.5.4/`, `v0.5.5/` and `v0.5.6/`) and a CI test (`packages/engine/tests/conformance_coverage.rs`) that fails when a schema operation is not covered by a conformance level. The full multi-version suite envisioned here is not complete: the coverage test looks up only the newest manifest, `conformance/v0.5.6/manifest.json`, while the two consistency tests run over every checked-in manifest.
