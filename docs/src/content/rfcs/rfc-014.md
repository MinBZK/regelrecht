...
> **Implementation note.** The core of this RFC exists: conformance manifests (`conformance/v0.5.0/`, `v0.5.4/`, `v0.5.5/`, `v0.5.6/`, `v0.5.7/` and `v0.5.8/`) and a CI test (`packages/engine/tests/conformance_coverage.rs`) that fails when a schema operation is not covered by a conformance level. The full multi-version suite envisioned here is not complete: the coverage test looks up only the newest manifest, the one for the version `schema/latest` points at, while the two consistency tests run over every checked-in manifest.
...
Today, the RegelRecht engine's correctness is verified through BDD tests (`bdd/conformance/*.feature` and `corpus/regulation/**/scenarios/*.feature`) using cucumber-rs. These are Rust-specific. A third-party Java or Python engine cannot run them. The tests verify integration behavior (loading regulations from the corpus, resolving cross-law references), not isolated schema-level operations. There is no artifact that says "if your engine supports schema v0.5.1, these are the exact inputs and outputs it must produce."
...
- **BDD tests** (`bdd/conformance/*.feature` + `corpus/regulation/**/scenarios/*.feature` + `packages/engine/tests/bdd/`): end-to-end scenarios using real regulation YAML from the corpus. Rust-specific (cucumber-rs). Cover realistic scenarios (zorgtoeslag, bijstand, erfgrensbeplanting) but do not test each operation in isolation.
...
The suite is stored under `conformance/` in the repository:
...
3. No operation appears in more than one level
...
1. For each Gherkin scenario in `bdd/conformance/*.feature`, extract the regulation YAML, input parameters, and expected outputs
...
- [RFC-012: Markings](/rfcs/rfc-012), handling constructs beyond engine expressiveness (Advanced conformance level)
