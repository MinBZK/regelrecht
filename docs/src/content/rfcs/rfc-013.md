...
>
> The tradeoff below states that trace depth is configurable. It is not: `TraceBuilder` offers `new`, `new_untimed` and `disabled`, with nothing between. [RFC-039](/rfcs/rfc-039) withdraws the claim and puts the folding of a subtree with the renderer. RFC-039 also gives the `trace` field of this receipt its shape, which is left empty in the envelope example.
>
> Not built: the engine always emits `accepted_values` empty (`packages/engine/src/service.rs`), so the cross-organization chain is a declared field without content; nothing signs a receipt or an accepted value; and the current engine version (0.3.0 in `packages/engine/Cargo.toml`) has no `engine-v` release tag, the latest being `engine-v0.2.0`, so a receipt cannot yet be replayed against a published binary.

## Context

The engine produces deterministic results: given the same regulation YAML, the same inputs, and the same calculation date, it always produces the same outputs. [RFC-006](/rfcs/rfc-006) chose Rust for this guarantee. Determinism within a single execution is not enough. Government agencies must be able to reproduce a specific decision months or years later, with the exact same result.
...
- **EU AI Act Art. 12** (mandatory August 2026): high-risk systems must automatically log events including input data and verification results. Logs must be retained for at least 6 months. (Regulation (EU) 2026/1744 has since moved that date to 2 December 2027 for high-risk systems under Annex III.) This applies only if the system is a high-risk AI system in the first place. The position paper argues that a deterministic engine executing rules adopted by officials is arguably not an AI system under Art. 3(1) at all ([Rules as Executed, section 7.3](/research/rules-as-executed#sec:european)); the logging this RFC specifies does not depend on the answer.
...
```json
{
  ...
  "accepted_values": [
    {
      "output": "inkomensgegeven",
      "value": 3200000,
      "authority": "inspecteur",
      "engine": "regelrecht",
      "engine_version": "0.5.1",
      "regulation_id": "algemene_wet_inzake_rijksbelastingen",
...
{
  "api_version": "1.0",
  "law_id": "algemene_wet_inzake_rijksbelastingen",
  "article": "21",
  "output": "inkomensgegeven",
  ...
}
...
The request asks for the income datum (*inkomensgegeven*), not the toetsingsinkomen. The Wet op de zorgtoeslag uses toetsingsinkomen (art. 2(2)) without defining it. The Awir defines it (art. 2(1)(i) and 8(1)) as the income datum for the calculation year, with income not taxable in the Netherlands added under art. 8(2), and AWR art. 21(e) says what that datum is: the verzamelinkomen where an income tax assessment is set, otherwise the belastbaar loon. What another organization accepts is that datum as the inspector (*inspecteur*) records it in the Basisregistratie Inkomen (AWR art. 21a), an authentic datum; art. 2.18 IB 2001 defines only one of its two forms.
...
The compliance requirements in this RFC (provenance fields, schema immutability) are themselves rules about how the system must behave. A platform that executes machine-readable law raises the question whether those rules can be expressed as RegelRecht regulations.
...
```json
{
  "results": {
    ...
    "output_provenance": {
      ...
      "bezwaartermijn_weken": { "type": "Override", "law_id": "vreemdelingenwet_2000", "article": "69" }
    }
  }
}
```
...
| `packages/engine/src/receipt.rs` | New: `ExecutionReceipt`, `ExecutionReceiptProvenance` types |
...
- **2026-09-25.** The example scope listed a source `amsterdam`, which has no role in the zorgtoeslag; it is now `minfin-central`, one of the per-ministry sources the review suggested (whether such sources will exist is open in [RFC-010](/rfcs/rfc-010)), and the receipt no longer carries a municipal scope. The inter-engine request asked Wet op de zorgtoeslag art. 2 for the toetsingsinkomen, which that act uses without defining; it now asks for the income datum of AWR art. 21(e), and the accepted value in the receipt is that datum rather than a toetsingsinkomen from the IB 2001, and a note explains that the Awir (art. 2(1)(i) and 8(1)) defines toetsingsinkomen as that datum. The provenance example attributed the four-week objection period of Vreemdelingenwet art. 69 to Awb art. 6:7 as a reactive output; it is a lex specialis override and is now shown as `Override`. The engine reported it as reactive; that bug is fixed in [#1616](https://github.com/MinBZK/regelrecht/pull/1616). The AI Act bullet now says it applies only to high-risk AI systems, which this engine arguably is not. Source: review by Mariette Lokin, [#1102](https://github.com/MinBZK/regelrecht/pull/1102).
