...
>
> [RFC-044](/rfcs/rfc-044) §6 applies two ideas of this RFC outside the engine. A decision the cell runtime records carries `inputs`, every parameter with its value and provenance (the `accepted_values` idea, for every parameter rather than only cross-organizational ones), and a `receipt` listing the loaded regulations and the cell's chronicle streams with one hash over both, the generalisation of `loaded_regulations` that RFC-022 §1.3 announced. The engine's own receipt is unchanged.
