...
> **Superseded by [RFC-031](/rfcs/rfc-031) as of schema v0.7.0.** That version
> renames this channel to `markings`, replaces `construct`/`suggestion`/`kind`
> with `about`/`reason`/`resolution`/`resolved_by`/`target`, and folds the four
> flag fields into two. The text below describes `untranslatables` as it stood
> when this RFC was accepted, and it stays that way: a law file on schema
> v0.5.x carries that field and the engine still reads it. Read RFC-031 for
> what a file on v0.7.0 and later carries.

> **Implementation note.** This RFC is implemented: the engine supports all four untranslatable modes (`UntranslatableMode` in `packages/engine/src/types.rs`), the schema carries `untranslatables`, and `bdd/conformance/untranslatables.feature` covers it.
...
[Mariette: zie voor een real life voorbeeld artikel 12, tweede lid, Algemene kinderbijslagwet: https://wetten.overheid.nl/jci1.3:c:BWBR0002221&hoofdstuk=III&paragraaf=1&artikel=12&z=2026-07-01&g=2026-07-01. In het tweede lid, tweede volzin zit een afronding. De engine moet niet alleen 'aanslaan' op afronden op, maar ook afgerond op. Ook van belang is dat afrondingsregels (en vast ook 'hoe om te gaan met' andere (open) termen vaak is opgenomen in interne (beleids)documenten. De BD heeft bv. een afrondingsbeleid. Kan nog een ander voorbeeld van een intranslatable worden opgenomen? Het feit dat het een open norm/term is, is niet doorslaggevend toch (daar zijn immers al andere oplossingen voor gemaakt)? Er moet meer aan de hand zijn, maar wat?]

[NB, zie ook artikel 12, derde lid, AKW, met een bijzondere 'geneste' vorm van overgangsrecht; niet relevant voor dit onderdeel, maar miss wel voor de implements] 
