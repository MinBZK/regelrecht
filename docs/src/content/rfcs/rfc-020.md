...
- **Overgangsrecht: eerbiedigende werking.** The old law stays applicable to existing cases
  ...
  example A). Eerbiedigende werking is not the only form of transitional law: *terugwerkende
  kracht* (retroactive effect), where new law governs a period before its entry into force, is one
  too. This RFC covers the first; retroactivity is open ([#1006](https://github.com/MinBZK/regelrecht/issues/1006)).
- **Statische verwijzing** (Aanwijzing 3.47): a static reference freezes the text
...
- **Ex nunc / ex tunc**: the heroverweging in bezwaar (Awb art. 7:11) is in beginsel ex nunc, with
  exceptions; which event date governs follows from the transitional law of the law being applied,
  with ex nunc, which case law derives from art. 7:11, as the default.
...
The event dates already exist: [RFC-008](/rfcs/rfc-008) ("the lifecycle is law") provides
`aanvraag_datum`, `besluit_datum`, `bekendmaking_datum` as stage inputs. The missing piece is a way
...
|---|---|---|
| Awb art. 7:11 (heroverweging) | [BWBR0005537](https://wetten.overheid.nl/BWBR0005537) | lid 1: "*Indien het bezwaar ontvankelijk is, vindt op grondslag daarvan een heroverweging van het bestreden besluit plaats.*" Case law derives the ex nunc starting point from this duty to reconsider; transitional law or the nature of the decision can displace it (example A) |
...
| Aanwijzingen 3.47 / 5.60 / 5.61 / 5.64 | [BWBR0005730](https://wetten.overheid.nl/BWBR0005730), deep links via [kcbr.nl](https://www.kcbr.nl) | drafting conventions: statische/dynamische verwijzing, plaats overgangsrecht, onmiddellijke werking, eerbiedigende werking (a circulaire of the Minister-President, amended by regelingen; instructions to the civil service, without external effect) |
...
| Awb bezwaartermijn chain | `corpus/.../algemene_wet_bestuursrecht/1994-01-01.yaml`, `corpus/regulation/nl/wet/vreemdelingenwet_2000/scenarios/bezwaartermijn.feature` | existing modeled lifecycle this RFC builds on |

### 1. Three temporal axes

Conflating these is the root confusion:

| Axis | Question | Where it is expressed |
|---|---|---|
...
### 3. The applicable date is computed by law
...
- **There is no single statutory "applicable-date" article**:
  ...
  - the overgangsrecht default is **onmiddellijke werking** (Aanwijzing 5.61); its exceptions are
    set out in the specific law's overgangsbepalingen (Aanwijzing 5.60).
...
- **The selector is stated in the law**: the engine provides per-reference `as_of` + an operation;
...
original *besluit* moment (ex tunc) is selected by which event date is passed as `as_of`. The
ABRvS derives the starting point from the duty to reconsider in art. 7:11: the heroverweging is in
principle based on the law and policy in force at that moment, ex nunc (*Greenpeace*, r.o. 6.2.3).
Transitional law, or a transitional rule in the policy, can displace that (r.o. 6.2.3), and so can
the nature of the decision, for instance where the law ties it to a reference date (r.o. 6.2.4).
Here that is the Vreemdelingenwet's own overgangsrecht; where it has none, ex nunc applies.
...
for a fixed fine the lower amount is favourable) is stated in the law. Stays within the existing
...
Extends the cucumber-rs harness (`corpus/regulation/nl/wet/vreemdelingenwet_2000/scenarios/bezwaartermijn.feature`, `bdd/conformance/einddatum.feature`):
...
- **Composes with [RFC-019](/rfcs/rfc-019)**: a static `as_of` can resolve an earlier version even
  after the target's `valid_to`; an ended reference at the default date still fails with the
  RFC-019 §3 diagnostic, stated relative to the resolved date.
