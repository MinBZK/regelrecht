...
A property's `description` is not a courtesy to future maintainers. It is how the schema explains itself, and it is expected to be present. Where an example clarifies more than a sentence, the property carries `examples` drawn from the real corpus rather than invented.
...
Schema v0.6.0 applies this to the existing schema: it adds 48 descriptions and 30 examples and nothing else, taking property coverage from 76% to 97%. The remaining gaps are `if`/`then` internals, which are machinery rather than fields a law author writes.
