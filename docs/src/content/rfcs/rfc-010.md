...
The corpus (`regulation/nl/`) is part of the regelrecht repo, alongside the engine, pipeline, admin, and editor. That works for an MVP, but doesn't match the legislative reality: regulations are enacted both on central and decentralized level. The constitutional legislator (government and parliament together), ministries, municipalities (*gemeenten*), provinces (*provincies*), water boards (*waterschappen*) each produce their own regulations, the latter four often filling in powers delegated or attributed by formal laws.

[RFC-003](/rfcs/rfc-003) (Inversion of Control) introduces IoC with `open_terms` and `implements`. This e.g. lets a municipality (*gemeente*) fill in a formal law without modifying that law. This goes for other delegated legislators. Technically this already works cross-repo, as long as the engine loads all relevant formal laws. The infrastructure for that is missing:

- **Discovery**: how does the engine find regulations based on delegation or attribution, either on a central of decentralized level? [Mariette: mi. geldt dit ook op centraal niveau, maar het is nog prangender voor decentrale regelingen, omdat er geen goede koppeling is tussen wetten.nl en lokaleregelgeving.nl; het is mi. een harde randvoorwaarde dat die koppelingen beter worden; BZK zou hiervoor als beleidsverantwoordelijke voor bekendmaking c.a. initiatief moeten nemen (zit nu wel wat in het opschalingsticket, maar vraag is of dat voldoende is)] 
...
- **Write-back**: how can a delegated or attributed legislator maintain regulations in their own repo via the editor? [Mariette: dit geldt mi. ook voor de formele wetgever?]
...
External sources are any Git repository that follows the required directory structure. They don't need to be forks or clones of the central corpus. A municipality (*gemeente*) like Amsterdam creates a repo `gemeente-amsterdam/regelrecht-amsterdam` containing their municipal regulations that declare `implements` on laws from the central corpus. [Mariette: is de gemeentelijke repo dan een directe afslag van de gemeentelijke verordeningen in lokaleregelgeving.nl? Je wil geen kopieën...] 
...
Each YAML file must:
- Conform to the RegelRecht YAML schema (referenced via `$schema`) [Mariette: is er ook al een format om een decentrale verordening van een EU-verordening te onderscheiden?] 
...
A YAML manifest describes which sources the engine should load: [Mariette: ik neem aan dat minbzk-central een voorbeeld is, en er ook een minszw-central, minfin-central is, voor de wetgeving waarvoor zij eerstverantwoordelijk zijn. Al die wetgeving maakt onderdeel uit van wetten.nl, zijn het daar 'uitsnedes' van (lijkt me wel fijn ivm. versiebeheer)?] 
...
sources:
  ...
  - id: local-tests
    ...
    priority: 100
```
[Mariette: wat bepaalt de priority?] 
**Scopes:**
...
A source without scopes (like the central corpus) delivers regulations without jurisdictional restrictions. A source can have multiple scopes, for example a province (*provincie*) that delivers regulations for multiple scope types:
...
scopes:
  ...
  - type: waterschap_code
    value: "WS0155"
```
[Mariette: Een provincie kan geen verordening voor een waterschap 'leveren'] 
**Priority:**

Multiple sources may provide a regulation with the same `$id`. This is allowed and expected: a local development source may override a central regulation for testing, or a municipality (*gemeente*) may provide a patched version of a central regulation during a transition period. [Mariette: Mi. kan dit niet. Een gemeentelijke verordening (of een gedelegeerde regeling) kan een formele wet nl. niet veranderen (zie ook RFC-003). Er kunnen alleen regels worden gesteld ter uitvoering/invulling van die wet] **Lower priority value = higher priority**, the law from the source with the lowest priority number is the one the engine uses. Think of it as rank: priority 1 outranks priority 10.
...
- **Patches**: a municipality (*gemeente*) may temporarily override a central law with a corrected version by using a lower priority number. [Mariette: dit kan mi. ook niet. Zie vorige opmerking] 
