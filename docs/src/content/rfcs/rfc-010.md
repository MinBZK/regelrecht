---
title: "RFC-010: Federated Corpus"
status: Accepted
...
The corpus (`regulation/nl/`) is part of the regelrecht repo, alongside the engine, pipeline, admin, and editor. That works for an MVP, but doesn't match the legislative reality: regulations are enacted at both central and decentral level. The formal legislator (*wetgever in formele zin*: government and States General jointly), ministries, municipalities (*gemeenten*), provinces (*provincies*), water boards (*waterschappen*) each produce their own regulations, the last four often exercising powers delegated or attributed by formal laws.

[RFC-003](/rfcs/rfc-003) (Inversion of Control) introduces IoC with `open_terms` and `implements`. This lets, for example, a municipality (*gemeente*) fill in a formal law without modifying that law, and the same holds for other delegated legislators. Technically this already works cross-repo, as long as the engine loads all relevant formal laws. The infrastructure for that is missing:

- **Discovery**: how does the engine find regulations based on delegation or attribution, at central or at decentralized level?
...
- **Write-back**: how can a delegated or attributed legislator maintain regulations in their own repo via the editor?
...
- Have a `$id` that identifies the regulation (does not need to be globally unique - see Priority below for conflict resolution when multiple sources provide the same `$id`)
- Declare `implements` (per [RFC-003](/rfcs/rfc-003)) when filling in open terms from a higher-level regulation
...
Example: you want to point the Amsterdam source to your own fork and add a directory with test regulations. Put this in `corpus-registry.local.yaml`:
...
Scopes are **claims**: a source declares which jurisdiction(s) it provides regulations for. A scope is an ownership declaration ("this source contains regulations from the municipality (*gemeente*) of Amsterdam"). It does not route requests; the engine never selects a source because someone asked for GM0363.

The engine uses scopes for two things:

1. **Validation** - if a law from a source with scope `gemeente_code: GM0363` declares `gemeente_code: GM0518` in the regulation itself, the engine generates a warning. The source claims to be Amsterdam but delivers a law for a different municipality (*gemeente*).
...
A source without scopes (like the central corpus) delivers regulations without jurisdictional restrictions. A source can have multiple scopes, for example a regional source that delivers both provincial and water board regulations, each under its own scope. A province itself cannot deliver a water board's ordinance; the source here is a publisher, not the legislator:
...
Multiple sources may provide a regulation with the same `$id`. This is allowed and expected: a local development source may override a central law for testing, or a municipality (*gemeente*) may provide a patched version of a national law during a transition period. **Lower priority value = higher priority**, the regulation from the source with the lowest priority number is the one the engine uses. Think of it as rank: priority 1 outranks priority 10.
...
When two sources have equal priority and the same `$id`, the engine raises an error at load time. Detection happens when sources are fetched and indexed, so a misconfigured source surfaces at startup instead of when a citizen's request hits it.
...
The engine already supports multi-version regulations via `valid_from` and `reference_date` (see `resolver.rs`). In a federated model this becomes more important: when executing a regulation as it was on 2025-01-15, you need all sources at that date, at all levels: central (formal and delegated) and decentral (autonomous, attributed or in co-government).
...
Within each source, the existing `valid_from` filtering still applies: the engine selects the law version valid at the `reference_date`. The two mechanisms work together: Git tags give you the right file versions, `valid_from` gives you the right regulation versions within those files.
...
**Scope validation:** scopes are claims, not hard-enforced boundaries. A regulation from a source with scope `gemeente_code: GM0363` that declares `gemeente_code: GM0518` generates a warning, not an error. The engine trusts the source but signals deviations.
...
"source is not configured for write-back" message. We do not silently
degrade to a local-only commit (the user would think the
...
- **Source badge**: each regulation shows a badge with the source name (e.g. "MinBZK Central" or "Gemeente Amsterdam")
...
- **Cross-source `implements` visualization**: show which municipal regulation fills in which formal law
- **Save to repo**: button that opens a commit dialog (choose branch, commit message, create PR)

### Open questions from a legal review

Raised by Mariette Lokin in [#1099](https://github.com/MinBZK/regelrecht/pull/1099):

- **Discovery depends on the publication platforms.** Finding regulations based on delegation or attribution is hard at central level and harder at decentral level, because wetten.overheid.nl and lokaleregelgeving.overheid.nl are not properly linked. In the reviewer's view better links are a hard precondition, and BZK, as the ministry responsible for official publication, should take the initiative.
- **Copies or slices?** Should a municipal repository be a direct derivative of the ordinances on lokaleregelgeving.overheid.nl rather than a copy? And should a central source per ministry be a slice of wetten.overheid.nl, which would help version management?
- **Write-back for the formal legislator.** Does maintaining regulations through the editor apply to the formal legislator too, not only to delegated and attributed legislators?
- **Can a municipality patch a national law?** The priority section above lets a municipality provide a patched version of a national law. The reviewer points out that a municipal ordinance cannot change a formal law: it can execute or fill in that law, or supplement it where the law leaves room (Gemeentewet art. 121), but it cannot replace it. A municipality that executes a national law can publish its own interpretation of it (see [Rules as Executed, section 4.3](/research/rules-as-executed#sec:whochooses)), but that interpretation does not replace the law for everyone else. Whether "patched version" meant a corrected machine-readable model rather than a changed law, and whether priority should be allowed to do this at all, is not decided here. The loader warns when a scoped source provides a national regulation, but accepts it. Discussed in [#1621](https://github.com/MinBZK/regelrecht/issues/1621).
- **EU regulations (answered).** Is there a way to tell a decentral ordinance from an EU regulation? The schema already distinguishes an EU regulation from a decentral ordinance through `regulatory_layer` (`EU_VERORDENING`, `GEMEENTELIJKE_VERORDENING` and so on).
...
- **Scope validation as trust boundary**: without complex PKI infrastructure, the engine can validate via scopes that a source doesn't deliver regulations outside its jurisdiction.
...
- **Write path complexity**: branch management, merge conflicts, and PR handling in the editor is non-trivial. This is planned for a later phase.
...
Source picker, source badge per regulation, and direct GitHub access from the editor (via the GitHub API, not via the admin backend).
...
| `frontend/src/EditorView.vue` | Editor UI | +source picker, +badge, +write-back |
...
- **2026-09-25.** The scopes example no longer has a province deliver a water board's regulations. The text now speaks of regulations rather than laws where delegated and decentral regulations are meant, and distinguishes central from decentral, delegated from attributed: a municipal ordinance is not a law. The reviewer's objection to the "patched version" use of priority is recorded as an open question, not as a correction, because removing that use would change what the design does. Source: review by Mariette Lokin, [#1099](https://github.com/MinBZK/regelrecht/pull/1099).
