...
[RFC-003](/rfcs/rfc-003) (Inversion of Control) introduces IoC with `open_terms` and `implements`. This lets a municipality (*gemeente*) fill in a national law without modifying that law. Technically this already works cross-repo, as long as the engine loads all relevant laws. The infrastructure for that is missing:
