...
**`uri` is a convenience.** `anchor.law_id` and `yaml_path` are the address; the URI is nicer to read where it can be written, and often it cannot. The scheme puts the law id before a `/`, so a law whose id is `alcoholwet/vergunning/rotterdam` has no expression in it at all, and 30 of the 79 law ids in `corpus/demo` contain a slash; it also has nothing to put in the output segment for a step naming an input or an open term. It is then absent, and absence does not mean the step denotes no value.
...
Anchors will be sparse. Under `corpus/regulation/` the element-level reference object appears nowhere: the nine `legal_basis` occurrences there are four law-level authority lists and five plain strings on open terms. That form is used 346 times under `corpus/demo/`, where `explanation` occurs 344 times against never in `corpus/regulation/`. A step whose element cites nothing reports nothing rather than borrowing the enclosing article's citation, which leaves the gap visible.
