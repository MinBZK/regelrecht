...
The modeling question is narrow: **a law YAML must express the rounding and precision that is
prescribed in the regulation or in execution policy.** As with [RFC-023](/rfcs/rfc-023), the
starting point is the legislative situation, not the engine. The catalog is drawn from the current corpus.
...
rounding operation. This RFC supplies that mechanism, in keeping with RFC-012's own rule that the
vocabulary grows only when real legal texts demand it.
...
**6. Rounding often comes from execution policy, not from the law.** Where the law is silent, an
executing organization frequently has its own policy on how to round, and that policy can differ
between organizations, or even between parts of one. Point 4 is an example: the rule against
rounding intermediate values is Belastingdienst/Toeslagen policy. A legal review of this RFC
([#1287](https://github.com/MinBZK/regelrecht/pull/1287)) proposes a standard rounding mechanism
that only the law itself can override. For the rounding method, that standard is close to the Hoge Raad default
of point 4. Such a standard would itself be a platform default; to square it with point 5, it would
have to be declared and published, for instance in the Engine Policy of [RFC-015](/rfcs/rfc-015),
rather than left implicit in the engine. That leaves an open tension with per-organization policy: if such policy departs from the standard, it
has to be published and attributable as the executor's own specification, not hidden in an engine
default ([Rules as Executed, section 3.2](/research/rules-as-executed#sec:intelligibilitygap)), and
whether it may depart at all where the law is silent is not settled here.
...
2. **Introduce three distinct rounding operations**: `ROUND` (round to nearest), `CEIL` (round up),
   ...
   *not* read implicitly from the operand's `type_spec`, so the rounding instruction is
...
4. **Intermediate values keep full precision.** As a normative rule: nothing rounds a value between
...
- Modelers must now *think about* rounding explicitly, where a silent default used to cover it. The
  law's rounding becomes visible and auditable.
...
The same reasoning decided a later case. [RFC-032](/rfcs/rfc-032) adds `START_OF`, which cuts a date
back to the first day of its year or month, even though `DATE_PART` plus the existing `DATE` derives
it in nine lines with the cut hidden in the literal `day: 1`. That is an arithmetic chain
approximating a truncation, so the truncation gets a name. The mirror image is ruled out just as
firmly: `START_OF` may not become an implicit property of a date type, nor a silent normalization at
an operation boundary. The cut stands in the law YAML where the law prescribes it, for the same
reason this RFC took rounding out of the unit coercion of RFC-015.
...
- [RFC-001: YAML Schema Design Decisions](/rfcs/rfc-001): the type system and `type_spec` (the
  `type_spec.precision` field a rounding result targets is in the schema; see issue #444)
