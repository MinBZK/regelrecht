---
title: "RFC-001: YAML Schema Design Decisions"
status: Accepted
...
- No separate endpoint definition is needed. Every output is an endpoint
- **Naming pattern:** `^[a-z0-9_]+$` (e.g., `toetsingsinkomen`)

> **Amended by [RFC-038](/rfcs/rfc-038).** Every output stays callable by another
> law, which is what this decision settled and what cross-law execution rests on.
> Whether a portal should offer an output to a citizen is a separate question,
> left unanswered here; RFC-038 answers it and reintroduces the removed
> `discoverable` field for that purpose alone.
