...
> **Amended on 25 September 2026.** The aanvraag-cel proof of concept (`packages/cel`) and a review of it against the position paper changed four points of this draft. §1.2 gives "kroniek" one meaning. §1.3 describes the chronicle fields the runtime records and moves their schema to `schema/chronolex/`. §2 adds the process as the party that acts, next to the cell. §4.2 moves synthesis to the consumer, so a law no longer names a cell. [RFC-044](/rfcs/rfc-044) records the design of that runtime and the reasons for each change; the rest of this draft is as it was.
