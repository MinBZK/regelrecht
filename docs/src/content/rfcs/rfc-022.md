...
The RFC is neutral about any specific external standard at the schema level. The first integration it anticipates is the [Blauwe Knop Connect](https://vorijk.nl/standaard/connect/draft-bk-connect-00.html) standard behind Mijn Betaaloverzicht; concrete integration shape lives in the [CJIB pilot proposal](/concepts/cjib-blauwe-knop-source-proposal). Schema and engine work, plus the three reused-mechanism amendments to RFC-007/009/013, are specified in [RFC-025](./rfc-025); until they land, the resolver, signing, and provenance decisions in §4, §3.2 and §1.3 are accepted pragmatically.
...
This RFC adopts the [Chronolexografie position paper](https://chronolexografie.nl/position-paper/) vocabulary (*lexogram*, *decretogram*, *executogram*, *chronicle*, *reduction*, *lexostatus*), introduced in context where first needed; glossary entries land with RFC-025 Phase 3.
...
Two fields apply to chronolexograms regardless of type, sharing a pattern: each travels with the chronolexogram when it is signed, propagated, or surfaced. The third concern, *rechtsbescherming*, is a derivation through RFC-008, not a field (§3.3).
...
```yaml
# in the payments_received chronicle-stream from §1.3
events:
  - name: payment_received
    extensions:
      blauwe_knop:
...
Two properties of the `blauwe_knop` block matter at the architecture level; the full stage-binding and validation mechanics live in the [CJIB pilot proposal](/concepts/cjib-blauwe-knop-source-proposal) and RFC-025.
...
- **Stage-aware visibility, not emission.** Blauwe Knop is pull-based (a citizen-client authenticates and pulls on demand), so the RFC-008 binding is *visibility*, not emission: a *verplichting* becomes visible from `BEKENDMAKING` onward, because before notification it has no juridical existence to display. Because `visible_from_stage` has a legal consequence (the `bezwaar_route` einddatum is only populated from the route-populating stage), it is a closed-enum, validated field, not free-form, and serving a route-bearing decretype before that stage is not permitted. **Until the load-time check ships (RFC-025 Phase 2), the `blauwe_knop` extension is experimental** and a *production* security context must not activate the source for any route-bearing decretype; the CJIB pilot runs in a sandboxed context, which is what lets it exercise the Wahv source meanwhile.
...
1. **Awb-bezwaar route.** For an ordinary Awb-beschikking (Awb titel 4.4 *geldschulden*, most *bestuurlijke boetes*): a `bezwaar_route` whose *einddatum* the AWB-6:8 hook computes at BEKENDMAKING (the termijn cannot start before notification), complete only from BEKENDMAKING onward.
2. **Lex-specialis own-regime route.** The Wahv (Wet Mulder) disapplies Awb *bezwaar*: its remedy is *administratief beroep* before the *officier van justitie* (Wahv art. 6), then *beroep* before the *kantonrechter* (Wahv art. 9). The engine emits a `beroep_route` (not a `bezwaar_route`), selected from the `procedure_id`, never hard-coded per cell.
...
- **For executograms.** A *feit* carries no route by default. A specific executogram may, *because it implicitly references a nested besluit*: a `kwijtschelding_verleend` event in the same `payments_received` stream (§1.3) declares **`references_decision`** (an optional event field holding the `$id` of the BESCHIKKING it executes), and the engine derives the route from that decision's RFC-008 procedure-state. It is distinct from the §3.1 `modality.is_intrekking_van` backlink, which lives on a *besluit*.
...
```yaml
# cells/cjib/capabilities.yaml (sketch)
lexostatus_definitions:
  - name: openstaande_vorderingen
    description: >
      All open payment obligations for the given BSN, filtered on
      procedure-stage and net of withdrawals.
    inputs:
      - name: bsn
        type: string
    reduction:
      sources:
        - chronicle: cjib_main_chronicle   # the chronicle from §1.3
...
- [RFC-025: Implementatieplan chronolexogram-types](./rfc-025): schema, engine, and the RFC-007/009/013 amendments
