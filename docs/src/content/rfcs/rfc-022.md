...
This RFC **does not** introduce a `bezwaarbaar` field (Awb: *voor bezwaar vatbaar*); RFC-008 already models the AWB lifecycle a per-rule field would duplicate (Alternative 7). The route from a chronolexogram back to its objection stage is derived through RFC-008 (§3.3).
...
- `BETALINGSVERPLICHTING`: generic financial obligation imposed by a *bestuursorgaan*. Strictly, the payment obligation follows from the *beschikking* rather than being one; the value names a kind of *beschikking* by its effect. Whether that makes it a category of its own, and whether an *aanslag* would then be one too, is open.
- `STRAFBESCHIKKING`: criminal-law settlement under Sv art. 257a
- `BESTUURLIJKE_BOETE`: sectoral administrative fine

To anchor these in recognisable cases: a Wahv parking or speeding fine (Wet Mulder), which art. 4 Wahv calls an administrative sanction imposed by *beschikking*, is a `BETALINGSVERPLICHTING`, with the Wahv's own *beroep*-route (art. 6 Wahv); an OM settlement for a (heavier) speeding offence routed criminally is a `STRAFBESCHIKKING`, with *verzet* before the *strafrechter*; an AP or ACM fine, or a fine from the tax inspector or ILT is a `BESTUURLIJKE_BOETE`, with the ordinary Awb-*bezwaar*-route. A financial fact can thus carry a different decretype and a different *rechtsbescherming*-route depending on the procedure that produced it.
...
Withdrawal or amendment of a *beschikking* does not by itself settle what happens to money already paid: reclaiming it takes a separate *terugvorderingsbeschikking*, which is a BESCHIKKING of its own.
...
- **Activation belongs to the security context.** Whether a deployment serves a Blauwe-Knop-source is a security-context decision (§2), not a property of the law. A deployment that runs the same Wahv as CJIB but serves no source (the Wahv *beschikking* a citizen receives from CJIB imposes the sanction, art. 4, on behalf of the officials the Wahv designates, art. 3; its collection follows a separate chapter of the Wahv) simply omits the `blauwe_knop_source` feature-block; the same lexogram works either way. The presence of an `extensions.blauwe_knop` block is a *hint* for deployments that do activate the source.
...
- The legal basis for inter-organisation data exchange (per-case, not engine-level), and the governance of which facts are *vastlegbaar* (which facts are recorded follows from the substantive law that specifies the data for the *beschikking*; this RFC fixes only the *mechanism*).
