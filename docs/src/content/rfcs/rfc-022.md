...
The RFC is neutral about any specific external standard at the schema level. The first integration it anticipates is the [Blauwe Knop Connect](https://vorijk.nl/standaard/connect/draft-bk-connect-00.html) standard behind Mijn Betaaloverzicht; concrete integration shape is set out in the [CJIB pilot proposal](/concepts/cjib-blauwe-knop-source-proposal). This RFC reuses three existing mechanisms (the RFC-007 `source.regulation` resolver, the RFC-013 Execution Receipt, and RFC-009 signing) in ways that ask for a small follow-up amendment to each; until those land, the resolver, signing, and provenance decisions in §4, §3.2 and §1.3 are accepted pragmatically. The RFC-007 and RFC-013 amendments are additive; the RFC-009 one raises an open security question (whether the cell's FSC key may also sign Blauwe-Knop responses), so the pilot uses a dedicated JWS signing key and treats key-reuse as a later optimisation, not a blocker.

This RFC **does not** introduce a `voor bezwaar vatbaar` field; RFC-008 already models the AWB lifecycle a per-rule field would duplicate (Alternative 7). The route from a chronolexogram back to its objection stage is derived through RFC-008 (§3.3).
...
- `BETALINGSVERPLICHTING`: generic financial obligation imposed by a *bestuursorgaan* [Mariette: is de boetebeschikking de betalingsverplichting, of vloeit die uit de beschikking voort? Dus moet dit wel een aparte categorie zijn? If so, dan zou een aanslag ook van het type 'betalingsverplichting' zijn.]
- `STRAFBESCHIKKING`: criminal-law settlement under Sv art. 257a
- `BESTUURLIJKE_BOETE`: sectoral administrative fine

To anchor these in recognisable cases: a Wahv parking or speeding fine (Wet Mulder) is a `BETALINGSVERPLICHTING`, with the Wahv's own *beroep*-route (artikel 6 Wahv) [Mariette: artikel 4 Wahv benoemt het als een administratieve sanctie die bij beschikking wordt opgelegd]; an OM settlement for a (heavier) speeding offence routed criminally is a `STRAFBESCHIKKING`, with *verzet* before the *strafrechter*; an AP or ACM fine, or a fine from the tax inspector or ILT is a `BESTUURLIJKE_BOETE`, with the ordinary Awb-*bezwaar*-route. A financial fact can thus carry a different decretotype and a different *rechtsbescherming*-route depending on the procedure that produced it.
...
Two earlier candidates are excluded: `INCASSO_BESCHIKKING` conflated an *executoriale titel* (the *dwangbevel*, Awb 4:115/4:116) with a primary beschikking; `INTREKKING_BESCHIKKING` treated a *modality* as a *type* (an *intrekking* is a nested besluit per RFC-008 Open Question 5, referenced via `modality.is_intrekking_van`, §3.1).
...
```
[Mariette: dit is toch geen automatisme? Intrekking of wijziging van een beschikking leidt tot een aparte terugvorderingsbeschikking] 
...
1. **Awb-bezwaar route.** For an ordinary Awb-beschikking (Awb titel 4.4 *geldschulden*, many *bestuurlijke boetes*, other Awb-beschikkingen (except UOV)): a `bezwaar_route` whose duration is the AWB-6:7 *termijn* (six weeks) but whose *einddatum* the AWB-6:8 hook fixes at BEKENDMAKING (the termijn cannot start before notification), complete only from BEKENDMAKING onward.
2. **Lex-specialis own-regime route.** The Wahv (Wet Mulder) has its own regime: its remedy is *administratief beroep* before the *officier van justitie* (Wahv art. 6), then *beroep* before the *kantonrechter* (Wahv art. 9). The engine emits a `beroep_route` (not a `bezwaar_route`), selected from the `procedure_id`, never hard-coded per cell.
...
- The legal basis for inter-organisation data exchange (per-case, not engine-level), and the governance of which facts are *vastlegbaar* (a policy matter; this RFC fixes only the *mechanism*). [Mariette: wat is bedoeld met 'vastlegbaar'? Dat volgt toch uit de desbetreffende materiewetgeving waaruit de gegevensspecs voor de beschikking voortvloeien?] 
...
**Alternative 7: A per-rule `voor bezwaar vatbaar` block on `produces`.** Rejected: RFC-008 already models *bezwaar* correctly, including that the *termijn* starts at BEKENDMAKING. A static `termijn_dagen: 42` cannot reflect a computed einddatum. This RFC's contribution is to require integrations to *include* the route (§3.3), not redeclare it.
