...
> **Amended on 25 September 2026.** The aanvraag-cel proof of concept (`packages/cel`) and a review of it against the position paper changed this draft in these places:
>
> - §1.2 gives "kroniek" one meaning (the paper's: the time-ordered collection a cell keeps), where it also named the stages of one besluit. It no longer says every stage is a stage-decretogram: a stage that is a decision is one, and a stage brought about by a factual act of the authority, such as the notification, is recorded as that act (a `handeling`).
> - §1.3 describes the chronicle fields the runtime records (two times, the case identifier, the stage, the list of legal grounds) and moves their schema to `schema/chronolex/`.
> - §2 adds the process as the party that acts, next to the cell, and the acting actor as a fourth axis.
> - The heading of §4 and the text of §4.1 no longer speak of reusing `source` or of *zaak-kronieken*; §4.1 orders a reduction on `op_moment` and then `vastgelegd_op`.
> - §4.2 moves synthesis to the consumer, so a law no longer names a cell; the old text is Alternative 10.
>
> [RFC-044](/rfcs/rfc-044) records the design of that runtime and the reasons for each change. Everything else in this draft is unchanged.
...
4. Cross-cell queries, with the consumer naming the source cell in its own configuration and the runtime selecting the transport per context (Blauwe Knop for citizen-clients, FSC for authorised servers).

The RFC is neutral about any specific external standard at the schema level. The first integration it anticipates is the [Blauwe Knop Connect](https://vorijk.nl/standaard/connect/draft-bk-connect-00.html) standard behind Mijn Betaaloverzicht; the concrete integration shape is left to whichever deployment first activates a source. This RFC reuses two existing mechanisms (the RFC-013 Execution Receipt and RFC-009 signing) in ways that ask for a small follow-up amendment to each; until those land, the signing and provenance decisions in §3.2 and §1.3 are accepted pragmatically. The RFC-013 amendment is additive; the RFC-009 one raises an open security question (whether the cell's FSC key may also sign Blauwe-Knop responses), so the pilot uses a dedicated JWS signing key and treats key-reuse as a later optimisation, not a blocker.
...
1. **The corpus is the lexogram store.** Only normative content belongs in `corpus/regulation/`.
2. **The chronicle store is separate.** Registration specifications belong in `chronicles/`; a registration-spec is not a regulation.
...
A decretogram is **lifecycle-aware** through RFC-008. The `procedure_id` on `produces` selects the AWB procedure. The lifecycle is **not** one accumulating decretogram: the position paper makes each chronolexogram *elementary* ("in exact één chronolexogram"). The paper pairs this with a transactional-consistency rule that the elementarity citation alone does not carry: what arises *together* is recorded together in one chronolexogram ("wat tegelijkertijd ontstaat wordt samen vastgelegd", which "voorkomt multirealiteit"). Stage-splitting therefore applies to *successive* events in a besluit's lifecycle, not to facts that arise simultaneously: a single decretogram that imposes a *hoofdsom* plus an *opslag* plus *rente* records those co-arising elements as one elementary artefact, not three. Each RFC-008 stage (BESLUIT, BEKENDMAKING, BEZWAAR, ...) is its own elementary chronolexogram, and the stage grams of one besluit share a `zaakkenmerk`. A stage that is itself a decision (BESLUIT) is a stage-decretogram. A stage brought about by a factual act of the authority is recorded as that act: the notification (BEKENDMAKING) is the sending or handing over of the decision (Awb 3:41), neither a decision nor a delivery, and [RFC-044](/rfcs/rfc-044) §3 records it as a `handeling` that carries the stage. A stage that runs of itself, such as BEZWAAR after the notification, has no gram until someone acts in it. The stage grams do not form a *kroniek* of their own. In the position paper a kroniek is the time-ordered collection a cell keeps its chronolexograms in, with its own time axis (§2), and this RFC uses the word only in that sense: the stages of one besluit sit in a chronicle of the cell next to other grams, and the `zaakkenmerk` picks them out. The besluit stays the RFC-008 state container, and its current stage is its latest stage gram. [RFC-044](/rfcs/rfc-044) §4 describes the `zaakkenmerk` as a grouping in the registration, not a case abstraction. A case can hold several besluiten (an advance, a final determination, a recovery); RFC-044 §4 gives each a `besluitkenmerk` that its stage grams share, so each besluit goes through each stage once.
...
The runtime of [RFC-044](/rfcs/rfc-044) §3 fills in this sketch. `grondslag` is a list whose entries may name a paragraph, because an application rests on several provisions at once. An event has a `type` from the open vocabulary (§1), optionally a `soort` and an RFC-008 `stage`, and says whether its grams open or follow a case (`zaak`). A gram carries two times, the moment the fact holds (`op_moment`) and the moment the cell recorded it (`vastgelegd_op`), where this sketch binds one timestamp field.

Chronicle-stream files get a JSON schema of their own, apart from the law schema, in `schema/chronolex/`, so the two version independently. A regulation is normative content and a chronicle-stream is a registration spec; putting them in the same tree invites tooling to treat one as the other. The name `chronicles/` reflects "which events does the cell's chronicle accept"; a broader `cell-config/` is left to a future RFC.
...
- The **security context** holds the signing keys (RFC-009 §5), the trust material, and the authorisation under which a cross-cell query is answered. The paper has no security model at all; these concerns sit outside the cell. A deployment binds a security context to a cell (many-to-one in principle: a production and a sandbox context over comparable cells).

The cell does not act. In the paper a business process informs itself by combining lexostatuses from several cells (synthesis), draws conclusions, and has a cell record them. RegelRecht calls that party the **process**. It runs the engine and sends a cell what it concluded, and the cell decides whether that can be recorded. The actor a process acts for, in which role and through which channel, is recorded with each gram next to the recording actor and the competent authority ([RFC-044](/rfcs/rfc-044) §1).

They vary independently, which makes the sandbox case and the key/transport questions tractable. A RegelRecht engine is one component that can run against a cell, alongside others (a legacy system, a payment-intake system); RFC-009's one-engine / several-engines / mixed-component choices are unchanged.
...
| chronolexosynthese | the *cross-cell* activity: combining and judging lexostatussen from several cells, done by the process that consumes them | §4 |
| business process (*bedrijfsproces*) | the process: informs itself (synthesis), concludes (engine), has a cell record | RFC-044 §1 |
...
In all families the route is computed by the engine and states an *einddatum*, not a duration. The procedure, the field name (`bezwaar_route` vs `beroep_route` vs `verzet_route`) and the grondslag differ per family.

- **For executograms.** A *feit* has no route by default. A specific executogram may, *because it implicitly references a nested besluit*: a `kwijtschelding_verleend` event, in a sibling chronicle-stream joining the same `cjib_main_chronicle` (§1.3), declares **`references_decision`** (an optional event field holding the `$id` of the BESCHIKKING it executes), and the engine derives the route from that decision's RFC-008 procedure-state. It is distinct from the §3.1 `modality.is_intrekking_van` backlink, which applies to a *besluit*.
...
This RFC's contribution to *rechtsbescherming* is not a new field. It is the requirement that integrations include the route in their payloads, derived from the producing procedure.

### 4. Cross-cell queries: lexostatus, synthesis at the consumer, runtime chooses the transport
...
The reduction orders across the cell's *own* chronicles (not across cells, which is synthese) on a common axis the reduction makes explicit: the moment a fact legally holds (`op_moment`), and on a tie the moment the cell recorded it (`vastgelegd_op`), the two times every chronolexogram has ([RFC-044](/rfcs/rfc-044) §3 and §5). The runtime ships the result in the transport-specific format (FCID over Blauwe Knop, ACCEPT-payload over FSC); same reduction, different serialisations. A consumer cannot inject a custom reduction, it can only ask for a published lexostatus and supply documented parameters, which keeps the reduction logic with the cell that holds the chronicles.

#### 4.2 Synthesis at the consumer

The consumer names the cell it asks, and the law does not. A regulation that needs a fact another actor keeps declares a parameter, as it does for any value a caller supplies, and may say under which regulation the register holding it is kept (`origin: REGISTER`, [RFC-043](/rfcs/rfc-043)). The process that runs the regulation names in its own configuration which cell and which lexostatus supply that parameter, with which inputs, and maps the names the source uses to its own parameters ([RFC-044](/rfcs/rfc-044) §2). The source speaks the language of its own regulation. Translating is part of synthesis, and the paper puts synthesis with the consumer: "Synthese gebeurt bij de afnemer, niet bij de bron".

An earlier version of this section let `source.regulation` name a cell, resolved by the runtime after the loaded regulations, with a load-time error when a cell-id shadowed a regulation `$id`. That put the name of a deployment into a law, so one law read differently per deployment, and it had the engine combine lexostatuses from several cells, which is the consumer's work. It is kept as Alternative 10. The RFC-007 `source` block keeps its meaning: one regulation referring to another.

#### 4.3 Transport selection

Only **how the query travels** changes. Two transports, chosen by the context the engine runs in:
...
- The full cell-config format; this RFC introduces only `chronicles/` and the extension-activation site. [RFC-044](/rfcs/rfc-044) §8 describes the configuration of cells and processes in the proof of concept.
...
- **Activation belongs to the security context.** A *gemeente* runs the same Wahv lexogram as CJIB without forking it just because it does not emit FCID.
...
- **`extensions` blocks are uninterpreted at the schema level**, so a reader needs an integration document to know what a block means. The gain is that the schema says so explicitly.
...
- **Integrations must understand RFC-008 stages to decide visibility at the right moment** (a constraint on integration authors).
...
**Alternative 5: Keep the wrapper-regulation pattern (`procedureregeling_vorderingenoverzicht_rijk`).** Rejected: misusing a juridical genre name for an adapter invites confusion. Synthesis at the consumer (§4.2) is the right answer.

**Alternative 6: A single `source.kind: lexostatus_query` discriminator with fresh `cell:`/`lexostatus:` fields.** Rejected: it puts the cell in the law, as Alternative 10 does, and invents schema for it besides.

**Alternative 7: A per-rule `bezwaarbaar` block on `produces`.** Rejected: RFC-008 already models *bezwaar* correctly, including that the *termijn* starts at BEKENDMAKING. A static `termijn_dagen: 42` cannot reflect a computed einddatum. This RFC's contribution is to require integrations to *include* the route (§3.3), not redeclare it.

**Alternative 8: Treat every cross-cell query as a server-to-server FSC call.** Rejected: right for the *bevoegde-instantie* context, wrong for citizen-facing tools, where it would centralise data Blauwe Knop keeps decentralised. The runtime-selects-transport pattern (§4.3) keeps both contexts first-class.

**Alternative 9: Treat the Blauwe-Knop-source as a push-emit (`emit_at_stage`).** Rejected: Blauwe Knop is pull-based with no central destination. The stage-binding that matters is *visibility* at pull-time (§3.2); a push-based integration can define its own field in its own `extensions` namespace.

**Alternative 10: Let `source.regulation` name a cell.** The first version of §4.2: the runtime resolves the value against the data sources, then the loaded regulations, then the cells. Rejected on 25 September 2026: a law would carry the name of a deployment, and the engine would do the consumer's synthesis. The proof of concept showed that the translation between the source's names and the consumer's parameters belongs to the consumer as well (RFC-044 §2).
...
- [RFC-007: Cross-Law Execution Model](./rfc-007): the `source` mechanism, which an earlier draft reused for cross-cell queries (Alternative 10)
- [RFC-008: AWB Administrative Procedures](./rfc-008): lifecycle that decretogram emission and route derivation depend on
- [RFC-009: Multi-Organisation Execution](./rfc-009): federation, signing, trust material the security context holds
