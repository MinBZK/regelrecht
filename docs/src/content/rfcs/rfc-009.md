...
### Individual decisions (*beschikkingen*) versus calculations (*berekeningen*)
...
# or for the Immigration Service (IND), which decides in the Minister's name:
identity:
  name: "Immigratie- en Naturalisatiedienst"
  organisation_id: "00000000000000000000"  # OIN (placeholder)
  type: INSTANCE
  acts_for:
    - name: "Onze Minister"   # Vw 2000 art. 14(1)
      basis: MANDATE

# or for the Tax Authority (Belastingdienst), with two competent functionaries:
identity:
  name: "Belastingdienst"
  organisation_id: "00000000000000000001"  # OIN (placeholder)
  type: INSTANCE
  acts_for:
    - name: "inspecteur"      # AWR art. 11(1), assessment
      basis: ATTRIBUTION
    - name: "ontvanger"       # Iw 1990 art. 3(1), collection
      basis: ATTRIBUTION
```
...
The `name` in a `competent_authority` is the functionary or body that the law itself names, which is frequently something other than the organization running the engine. The Aliens Act 2000 (*Vreemdelingenwet 2000*) art. 14(1) assigns the power to grant, refuse and withdraw a fixed-term residence permit to the Minister (*Onze Minister*). The Immigration Service (*IND*) appears nowhere in that act and holds no power of its own; it decides by mandate (*mandaat*), and in law its decisions are the Minister's. The General Tax Act (*AWR*) art. 2(3)(b) and the Collection Act (*Invorderingswet 1990*) art. 2(1)(i) name two distinct functionaries inside one organization: the inspector (*inspecteur*) determines the assessment (*aanslag*, AWR art. 11(1)) and the collector (*ontvanger*) is charged with collection (Iw 1990 art. 3(1)).

`acts_for` lists the authorities an engine may act as, each with its legal basis: `ATTRIBUTION` where the law names the organization or one of its own functionaries, `MANDATE` where it acts in another body's name. Matching runs over that list. An IND engine therefore executes articles with `competent_authority: { name: "Onze Minister" }`, and a single Belastingdienst engine covers both inspecteur-articles and ontvanger-articles under one identity. This proposes an answer to the delegation (*mandaat/delegatie*) question [RFC-002](/rfcs/rfc-002) left open; it is a design proposal and not yet decided. The engine does not itself check that a mandate exists. Mandate decrees are published administrative law, the identity file asserts which authority it acts for, and the trace records that assertion so a reader can hold it against the decree.
...
FSC provides what multi-org law execution needs:
...
#### Local service registry for development
...
The **signature** makes the output authoritative. In simulation mode, the engine produces unsigned outputs. In authoritative mode, outputs are signed with the engine's identity. A consuming engine can verify that an accepted value was indeed signed by the competent authority.
...
#### Healthcare allowance (*zorgtoeslag*) without RegelRecht
...
#### Healthcare allowance (*zorgtoeslag*) with RegelRecht
...
#### Social assistance (*bijstand*) under the Participation Act (*Participatiewet*) without RegelRecht
...
#### Social assistance (*bijstand*) under the Participation Act (*Participatiewet*) with RegelRecht
...
### Data layer and legal basis for inter-org data exchange
...
In Dutch law, there is no general legal basis for one government organization to request data from another. Each inter-organizational data exchange requires its own specific legal basis, either through a base registry (*basisregistratie*) with mandatory use provisions (*verplicht gebruik*) in its own law, a domain-specific law (e.g., Wet SUWI art. 62 for the work and income domain, where a legal review suggested art. 54 instead and which article fits best is open ([#1298](https://github.com/MinBZK/regelrecht/pull/1298)); or Participation Act (*Participatiewet*) art. 64 for social assistance), or a bilateral statutory provision.

The policy principle of "single request, multiple use" (*eenmalige uitvraag, meervoudig gebruik*) aspires to prevent citizens from having to provide the same data to multiple government organizations. Inside the base registries (*basisregistraties*) this is legally enforced through per-registry mandatory use provisions, though the obligation reaches only data designated as an authentic datum (*authentiek gegeven*): Wet BRP art. 1.7(1), Kadasterwet art. 7k(1) and Handelsregisterwet 2007 art. 30(1) each oblige an administrative body (for the Handelsregisterwet, one within the meaning of Awb art. 1:1(1)(a)) to use the registry value where the information it needs is available there in that form. All three also provide a way out. An organization that doubts a value and files a correction report (*terugmelding*) may use another value from that moment on, without waiting for the registry to mark the datum as under investigation (Wet BRP art. 1.7(2)(b), Kadasterwet art. 7k(2)(b), Handelsregisterwet 2007 art. 30(2)(b)). For data outside the base registries, it remains a policy aspiration without a general legal mechanism. There is no general statutory obligation for one government organization to share data with another.
...
A supplied value can also be correct and still be the wrong value. The reviewer observed that the income the Tax Authority can supply for a benefit is the income of year t-2, while the benefit is determined over year t, and that, as far as she knows, in practice the available registry value (*BRI*) is used where the law takes the current income as its basis, which the citizen supplies and corrects where needed. Whether the period a supplied value belongs to should be part of a `source`, and how the engine should tell a law that prescribes t-2 from a practice that uses it, is tracked in [#1031](https://github.com/MinBZK/regelrecht/issues/1031).
...
- **Derived from law.** The execute/accept boundary comes from `competent_authority` in the law YAML, not from per-reference configuration. Adding a new law doesn't require configuring execution boundaries.
...
- **Verifiable.** Any organization can verify an accepted value by switching to solo simulation and re-executing locally. This serves the duty of care in Awb art. 3:2 (*zorgvuldigheidsbeginsel*), which requires an administrative body preparing a decision to gather the knowledge it needs about the relevant facts. Awb art. 3:9 does not apply here. It sits in the section on advice (*advisering*) and binds a body to check the work of an adviser as defined in art. 3:5(1), someone charged by statute with advising on decisions and working outside that body's responsibility. A supplying administrative body meets neither condition. The term *vergewisplicht* is also used in a different context, the duty to verify that a BSN belongs to the person (art. 12 Wet algemene bepalingen burgerservicenummer).
...
- Propagate W3C Trace Context (`trace_id`, `span_id`) across inter-engine calls, aligning with the Logboek Dataverwerkingen standard
