...
`acts_for` lists the authorities an engine may act as, each with its legal basis: `ATTRIBUTION` where the law names the organization or one of its own functionaries, `MANDATE` where it acts in another body's name. Matching runs over that list. An IND engine therefore executes articles with `competent_authority: { name: "Onze Minister" }`, and a single Belastingdienst engine covers both inspecteur-articles and ontvanger-articles under one identity. This closes the delegation (*mandaat/delegatie*) question [RFC-002](/rfcs/rfc-002) left open. The engine does not itself check that a mandate exists. Mandate decrees are published administrative law, the identity file asserts which authority it acts for, and the trace records that assertion so a reader can hold it against the decree.
