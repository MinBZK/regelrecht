...
`frontend-demo/src/data/entrypoints.js` holds the derivation, and the four places that read `discoverable` now call it: `portalLaws` and `findCase` in `demoStore.js`, `delegationProviders` in `delegation.js`, and a label in `ZaaksysteemView.vue`. The field is gone from all 49 law files that carried it.
