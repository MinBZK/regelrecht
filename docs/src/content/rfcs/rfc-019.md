...
- **The most recent version of any law is treated as valid forever.** A law can only "end" by being
  superseded - never by expiring or being repealed. (Most of what follows holds for delegated
  regulations as much as for formal laws.)
- **Real laws do end without a successor**, in two ways:
  - *intrinsic* - the law's own horizonbepaling: the Subsidieregeling bekostiging plafond
    energietarieven kleinverbruikers 2023 (CEK23) "*vervalt met ingang van 1 januari 2027*"
    (art. 8.1 lid 2). The Wet op de openluchtrecreatie is an intrinsic case too, though it looks
    extrinsic: the act that phased it out (Stb. 2005, 308) gave its article 44 the text "*Deze wet vervalt
    op 1 januari 2008*", so from then on the law itself set the date on which it lapsed (the
    same act also repealed single articles, which is article repeal);
  - *extrinsic* - another instrument ends it: article 2.1 of the Aanvullingswet bodem Omgevingswet
    withdraws the Wet bodembescherming ("*De Wet bodembescherming wordt ingetrokken*"). The
    Tijdelijke wet maatregelen covid-19 is a special extrinsic case: its measures lapsed per 20 mei
    2022 after the Eerste Kamer rejected the goedkeuringswet for the fifth verlenging (17 mei 2022,
    dossier 36.042), a law approving a royal decree (*koninklijk besluit*) that was needed for the
    measures to continue (art. VIII lid 4 as it then read).
- **Dependents break silently ("verweesde regelingen").** A regulation whose delegation basis
  ...
  Aanwijzing 6.24 prescribes explicit mede-intrekking instead - Stb. 2018, 362, the besluit that
  withdraws the Fokkerijbesluit and the Fokkerijregeling, executes it to the letter). Example: the
  Tijdelijke regeling maatregelen covid-19 was vervallen per the same date as the Tijdelijke wet
  maatregelen covid-19. In regelrecht's `source.regulation` model this is a dangling reference: the
  engine keeps computing on no-longer-valid law - a **silent wrong answer**.
...
references in terms of what the data proves.
...
| Wet op de openluchtrecreatie | [BWBR0006548](https://wetten.overheid.nl/BWBR0006548) · [Stb. 2005, 308](https://zoek.officielebekendmakingen.nl/stb-2005-308.html) | intrinsic end set by amendment: art. 44 "*Deze wet vervalt op 1 januari 2008*" |
| Aanvullingswet bodem Omgevingswet art. 2.1 | [BWBR0043277](https://wetten.overheid.nl/BWBR0043277) | extrinsic end: withdraws the Wet bodembescherming |
...
| Aanwijzingen voor de regelgeving 6.24/6.25 (old 243) | [BWBR0005730](https://wetten.overheid.nl/BWBR0005730) | drafting conventions for vervallen/intrekking (a circulaire of the Minister-President, amended by regelingen; instructions to the civil service, without external effect) |
...
- **Two provenance modes** for the date:
  - *intrinsic* - the law's own vervalbepaling (CEK23 art. 8.1 lid 2). The only mode RFC-019
    implements.
  - *extrinsic* - the date comes from another instrument (a law or a royal decree that withdraws it
    or makes it lapse, or open:
    ...
    with the terminating instrument as source of record. Withdrawal (*intrekking*) and lapsing
    (*vervallen*) are different acts in legislation: lapsing happens by operation of law, withdrawal
    is an explicit act. For `valid_to` the effect is the same, for its provenance it is not.
...
### 3. Expired references: grounded diagnostics
...
- **The staartzin is eerbiedigende werking** - the RFC-020 bridge case: after its instrument end,
  the regeling stays applicable to subsidies granted before 1 January 2027. `valid_to` correctly
  ...
  post-2026 date is RFC-020's `as_of` case.
...
The Tijdelijke wet maatregelen covid-19 shows what a static law-level `valid_to` does **not** cover
(§1 boundary):

- **The end date is not in the law.** Art. VIII lid 1: the inserted Hoofdstuk Va of the Wet publieke
  gezondheid (Wpg) *vervalt* three months after entry into force; lid 3 lets a KB extend it - or
  designated parts - by at most three months at a time ("*steeds ten hoogste drie maanden*"). After
  repeated verlengings-KBs the measures lapsed per 20 mei 2022, when the goedkeuringswet for the
  fifth verlenging was rejected. Authoring `valid_to: '2022-05-19'` would be **invented** - not
  grounded in the Twm's own text.
...
- The clean, grounded *intrinsic* case is CEK23 (example A): one literal vervaldatum for the whole
  regeling.
- A less special *extrinsic* case than covid-19 is the Wet bodembescherming, withdrawn as a whole
  by art. 2.1 Aanvullingswet bodem Omgevingswet: one withdrawing instrument, one date.
...
- Tested end-to-end in `bdd/conformance/einddatum.feature` (test laws `test_einddatum` /
  `test_einddatum_afnemer`).

## Testing

New cucumber-rs scenarios in `bdd/conformance/einddatum.feature` (test laws mirroring
...
- **Expresses laws that end.** Twm, CEK23 and WOR become representable; the latest version is no
  longer eternal.
- **No silent wrong answers.** A reference to an ended law is reported: an error when the reference
  is required, a propagating taint when it is optional.
- **Small and additive.** One field + an upper bound + one diagnostic; laws without `valid_to` are
  unchanged.
...
- **2026-09-25.** The Wet op de openluchtrecreatie was given as an example of an extrinsic end. It is intrinsic: the phasing-out act (Stb. 2005, 308) gave article 44 the text "*Deze wet vervalt op 1 januari 2008*" into the law itself. The extrinsic example is now article 2.1 Aanvullingswet bodem Omgevingswet, which withdraws the Wet bodembescherming; the covid-19 case is marked as a special one (approval by law of a royal decree). The Aanwijzingen voor de regelgeving are a circulaire of the Minister-President addressed to the civil service. Withdrawal and lapsing are now distinguished, CEK23 is named in full, and the abbreviations Twm and Wpg are written out. The intrinsic/extrinsic distinction itself is unchanged. Source: review by Mariette Lokin, [#1108](https://github.com/MinBZK/regelrecht/pull/1108); article 44 checked against the Staatsblad text.
