---
title: "RFC-007: Cross-Law Execution Model"
status: Accepted
...
**Lex specialis.** Awb 6:7 says "de termijn bedraagt zes weken", no exception clause, no delegation. Yet the Vreemdelingenwet artikel 69, eerste lid, says "in afwijking van artikel 6:7 van de Algemene wet bestuursrecht bedraagt de termijn voor het indienen van een bezwaar- of beroepschrift vier weken." It unilaterally replaces the value, and the general law does not know it is being overridden. That is not delegation ([RFC-003](/rfcs/rfc-003)). Delegated rules work out what the law leaves open and in principle cannot depart from it, unless the law explicitly allows them to.
...
The "Gelet op" row needs a caveat. The words stand in the preamble (*aanhef*) of a regulation, not in its articles, and the consolidated texts on wetten.overheid.nl show only the preamble of the original regulation. When a later amendment rests on a new delegation basis, that basis does not show there. See also the open questions in [RFC-003](/rfcs/rfc-003).
...
> **Amended by [RFC-041](/rfcs/rfc-041).** The two sentences above, and step 5
> of the resolution model below, hold for an override from another law. An
> article voiding an output of its own law applies whatever the contextual law
> is, and where more than one override applies the error names the laws they
> came from. The reasoning for scoping a replacement is unchanged; a void is a
> different claim.
...
|-----------|-------|--------|---------|--------|
| `DATE_ADD` | date + years/months/weeks/days | date | `2026-03-12 + 4 weeks → 2026-04-09` | Implemented |
| `DATE` | year + month + day | date | `DATE(2026, 4, 27) → 2026-04-27` | Implemented |
...
The examples below model only lid 1 of Awb 6:8 and of Vw 69, and cite the article as a whole. Both articles contain more rules about terms than the ones modeled here, so a citation should name the lid: a model that cites the whole article claims more than it covers.
...
- **2026-09-25.** The quote from Vreemdelingenwet art. 69 now follows the text of lid 1, and the objection-period chain names the lid (Awb 6:8 lid 1, Vw 69 lid 1). The lex specialis paragraph no longer compares an override with delegation as if delegated rules could also override; they cannot, unless the law allows it. Added a caveat on "Gelet op" as the marker of delegation and a note on citing the lid. Source: review by Mariette Lokin, [#1095](https://github.com/MinBZK/regelrecht/pull/1095).
