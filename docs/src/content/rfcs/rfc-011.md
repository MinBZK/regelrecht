...
The decision comes down to fit between what a language can express and what this project needs. The alternatives weigh up as follows.
...
4. **Proven viability.** A POC with 30+ laws demonstrates that the YAML approach models real legislation.
...
- A minimal feature set: only what the project requires
...
Conversion from established formats into RegelRecht YAML is technically feasible for features that fall within our YAML's scope. The evaluated formats (DMN, NRML, Datalog) share the core decision-modeling concepts (inputs, rules, outputs, conditions, temporal aspects) that map onto our constructs. Features outside our limited scope are not converted.
...
When stakeholders need it, conversion would let practitioners familiar with other formats contribute, would allow evaluation of existing law models, and would provide a migration path from legacy systems. The canonical YAML keeps the internal semantics clear.
...
**Prolog** (Adoption: Medium). Strong logical inference and backtracking, established in legal expert systems, with natural querying that finds all solutions automatically. But it has maintenance challenges at scale, less predictable performance than Datalog, and accessibility challenges for non-technical reviewers.
