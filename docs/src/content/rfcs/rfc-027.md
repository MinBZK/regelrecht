...
Executable logic. A `norm_gap`, where the norm is real and its content sits in a document the corpus does not hold. An `untranslatable`, where the norm is clear and the engine cannot express it. Or a `declares`, where the article fixes a document property rather than computing anything.
