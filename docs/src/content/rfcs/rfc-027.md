...
Legal translation is hard. The harder problem is that a wrong translation reads correctly. A duty modelled as a power, an establishing article given a competence that a later article confers, a deadline treated as fixed where the statute states a conditional outer bound, an article cited on a word match while its regime does not apply: each of these produces YAML that is well-formed, schema-valid, executable, and wrong in a way that only a legal expert notices.

This RFC decides what the enrichment process must do so that its output is checkable. Mechanically where the statutory text settles the matter, and by a legal expert where it does not. Checkability is the goal, because correctness at corpus scale cannot be established by reading, and can only be approached by making the places that need reading few and findable.

Issue #1036 records how enrichment works at the time of writing and where that falls short, with the measurements. That record belongs in an issue: it dates, and this document should not.

## Requirements

Seven properties any enrichment process must have. Each answers a failure mode that has been observed, and the design below implements them together.

**The text must be the law before a translation is judged against it.** A translation is checked word for word against the statutory text it was given. When that text is stale, truncated, paraphrased or contains an article the statute does not have, every judgement about the translation is void, including the favourable ones.

**The document structure must survive harvesting.** Chapter, division and paragraph headings are condensed legal classification written by the legislator, and they decide questions the article text alone cannot answer. Whether Awb 3:9 applies to a supplying administrative body turns on its place in the division on advice, where the adviseur definition of 3:5 lid 1 scopes it. A pipeline that drops the containers turns that error from likely into unavoidable.

**The agent must be handed what it needs.** An instruction the runtime cannot execute is worse than no instruction: it reads as a control that exists. Whatever the reading requires, sibling articles, definitions, delegation bases, explanatory memoranda, is assembled by the worker and placed in front of the agent.

**The reading must precede the writing, and be written down.** Classification is where this error class originates, so it needs an answer of its own, produced before the YAML. The agent that classifies must not know what the model looks like, because knowing it classifies towards what is easy to model.
...
**Time period absent.** The statute names a peildatum, a berekeningsjaar, a kalendermaand or a tijdvak, and the model computes without one. A source binding states no period at all, so a value established over one period silently answers a question about another.
...
**Invented value domains and dead structure.** An enumerated domain appears that no text fragment supports; outputs are declared that nothing consumes; a binding names no source at all. Beside these are defects that no per-article review can see, such as name collisions within one regulation and a concept swapped across a chain of laws.

The first two classes are the largest and both are mechanically detectable. That matters for sequencing, because the cheapest checks address the largest classes.

One qualification on the binding check, which follows from RFC-026. An unfulfilled binding is only a finding once the closure of the request is empty. While the article it points at is itself still queued for enrichment, it is an open task and not a defect. Measured on round 2 of the enricher archive, six of the seven binding findings on the zorgtoeslag were of that kind: artefacts of starting at the top. The law that was translated was not at fault.

## The chain

Enrichment is a chain of tasks. Every task ends at a gate, and a gate either
lets the work through or hands it back with what it found. A finding the
producer never sees changes nothing, and a finding that fails a job without
being shown teaches nobody.

Half the boxes need no model at all, which matters for cost and for trust. A
mechanical step gives the same answer every time and can be argued with, and
the model is spent where the work is a judgement.
...
does, because a success can be luck and a failure has a cause.
...
Every agent task has the same shape, and that shape makes the chain
...
and no binding states a period. The answer may be a change to the model, and
...
A third kind asks about the **record** instead of the translation, and it
comes last because the gates before it produce that record.
...
an operation. Schema v0.7.0 makes that check cheap, because each claim now has
its own field with one question: `markings` says the format cannot
express this, `open_terms` says the content is settled elsewhere. RFC-031 has the
...
and failing on it would punish the behaviour the design wants.

### Where the rounds are bounded

A round costs an agent run, so the bound matters. One repair round per gate is
the default, and a disagreement that survives it is recorded rather than fought
out. The reason is that a second disagreement is rarely a slip: it is either
something the text leaves open, which is what the marking channels
are for, or a defect in the instruction, which no amount of retrying inside one
law will fix. The post-mortem catches the second case, and it catches
it across laws instead of within one.

The number is set per gate rather than for the chain, because the gates differ
in what a second round could do: a schema error is repaired or is not, while a
coverage question can take a second look. Raising it is a measurement and not a
repair, and the measurement needs the counters of section 7 beside the finding
count, since a gate also clears when the agent declares more of the law
unmodellable. A round beyond the first is spent only when the previous one moved
something; RFC-028 states the three conditions that end the sequence early.
...
A marking says the format cannot express this. An open term says the law leaves the content open and something else fills it. A structural choice says the text did not determine which legal structure is right, and the agent chose. Three claims, three reviewers, so the third gets a separate list instead of an extra category in RFC-012.

Schema v0.7.0 has the first two and not the third. `structural_choices` was written into an earlier draft of that version, was read in one check and written by nothing, and was removed with the consolidation of RFC-031. A structural choice therefore goes into the enrichment sidecar next to the law, alongside the reading note and the gate results; open decision 3 below is settled that way for as long as the field is absent from the schema. The shape it takes there is unchanged:
...
**"In afwijking van X" names the rule departed from, and the displaced rule appears elsewhere in the sentence.** Article 3 reads "In afwijking van artikel 7, derde lid, van de Awir, bestaat geen aanspraak op een zorgtoeslag". The agent placed the override on Awir 7.3, which is what the sentence cites. The sentence displaces the entitlement that article 2 of this same law establishes, and it names that entitlement in its subject. The override therefore never fired, and the engine returned both the entitlement and the exclusion without complaint. Reading the sentence literally gives the wrong answer here, which is why this belongs in an instruction and not in the hope that the text explains itself.
...
The data model had no way to say that. An output has a value, and absence was indistinguishable from "nobody asked". Schema v0.7.0 therefore lets an override state that the output does not arise:
...
A consumer asking for a voided output gets an explicit failure carrying the ground, never a number. That is intended: an amount the law does not recognise must not be passed on silently.

However the question is asked. An article voiding an output of its own law applies whichever law the execution started from: asked through the law itself, asked through the granting law directly, or read across a law boundary by a third law. The scoping [RFC-007](/rfcs/rfc-007) puts on an override is right for a replacement, where how much you get may differ per the law invoking it, and wrong for a void, which is a statement about the entitlement and does not stop being true because a different law asked. An override from *another* law keeps that scoping, so a Vreemdelingenwet exclusion on an Awb term is still not a Participatiewet case.
...
The axis dividing the middle two is what has to change before the article is fully translated, and it decides whose queue the entry lands in. An open term is closed by a harvest or by a competent authority answering the individual case. A marking is closed by building the operation (`resolution: operation`) or by giving the format a shape it lacks (`resolution: model`). Schema v0.7.0 states this axis; earlier versions offered a `kind` on each of two fields, sorting by what kind of thing the gap was and leaving the owner implicit. That is the correction RFC-031 records; there the schema caught up with this RFC.
...
A `declares` entry therefore names a top-level property together with the value the article gives it, and a check holds the two against each other. When they disagree the article decides and the header is a stale copy. That leaves perhaps one article in a hundred, and finding nothing to write usually means the kind of provision has not been identified.
...
**The regression runner.** It measures the skill, so the checked-in corpus is irrelevant and every run generates fresh. Per gold record only the articles involved are regenerated with the candidate skill. Three assertion layers: the deterministic checks, the gold Gherkin scenarios against the fresh model, and a model judge with a fixed rubric for prose claims. Generation is not deterministic, so each record runs three times and takes the worst of three, which rewards a robust skill and not a lucky one.
...
Three guards against overfitting. A held-out split, where part of the records is not visible to whoever writes the skill and new records land in held-out first. A memorisation lint that fails when the skill text contains gold set article numbers, because a skill that names them has memorised the case and not the pattern. And the rule that a skill change must address a claim type rather than an individual record.
...
Two limits apply. Case law establishes ex tunc what the law always was, so a 2023 judgment about a 2015 provision is not anachronistic and excluding it makes the translation worse. And the model holds training knowledge from after every statutory date, which cannot be removed. Strict time correctness is enforceable at the source level and not at the model level.

None of the eleven was caused by anachronistic knowledge, so the measurement puts this here as a version fidelity requirement and not as a fix for the errors in this RFC. It remains worth enforcing, because a corpus that claims to hold the 1996 law should hold the 1996 law and the Zorgverzekeringswet file shows that it does not always.
...
The general rule this yields: the text always suffices to see that a choice is unsupported, and it does not always suffice to know what should stand instead. The measurement supports the marking route, because marked choices in the sample were almost never wrong and silent choices frequently were.
...
| 5 | Awb 3:9 against 3:2 | 2, 3, 4, 5 | Structural path shows "Afdeling 3.3 Advisering"; definition register records the scope of 3:5 lid 1; Q4 forbids grounding outside the regime; the reference lens reads the division heading |
...
| 8 | Mandate against delegation | Partially, 4 and 6 | The memo records the convention; the vocabulary rule converts the error into a marked choice. Not encodable as procedure |
...
How each failure class above is addressed, named in full because the classes were never given letters to refer back to.
...
Five classes have no line here. *Omission that runs one way* is not caught by any gate in this design, because its cause is the instruction and not the output. The other four have a deterministic gate each, built after this text was first written: *a displacement that never fires*, *a unit that changes at the law boundary*, *silence where a marking belongs* and *a restriction that restricts nothing*.
...
The roles are separate agent runs inside one job rather than separate job types. Three job types would clone the same law three times, hit the per-provider rate cap three times for one law, and collide with the uniqueness constraint on the job queue. The firewall is the process boundary and not the job type, and running several prompts with different rights inside one job is a pattern the pipeline already supports.
...
2. **Corpus: lid fragmentation.** Whether a lid remains a separate "article". As long as it does, reassembly is permanently required and every per-article approach pays that cost.
3. **Schema: `structural_choices`.** Settled for now by the sidecar, because schema v0.7.0 removed the field that briefly existed there. Whether it returns to `machine_readable` is open, and it costs a schema version because that object is `additionalProperties: false`. The sidecar keeps the law YAML schema-conformant and separates the marking from the diff it belongs to.
...
6. **The reading note as sidecar or schema.** The same argument as `.enrichment-result.yaml`: process metadata goes alongside, the law YAML stays clean.
7. **A write path for `accepted`.** The admin API exposes `GET /api/untranslatables` and no PATCH, in admin or in editor-api, and `replace_untranslatables` rewrites the table from the YAML per `(law_id, provider)` on every run; the endpoint and the table still use the pre-v0.7.0 name. `accepted: true` can therefore only be reached by editing the law file, which is the route actually being walked. Whether a write path is built is a separate decision from anything in this RFC.
...
**In the docs:** [Pipeline: Enrich Worker](/components/pipeline#enrich-worker) for the gates as they run, and [Markings](/concepts/markings) for the marking channel.
