  ...
  - id: beschikking
    ...
    stages:
      ...
      - name: BEHANDELING
        ...
        # Awb 4:13 lid 2: absent such a provision a reasonable term applies, which has in
        #   any case expired when, within eight weeks of receiving the application, no
        #   decision has been given nor a notification under Awb 4:14 lid 3 has been made
...
4. **Decision deadline (*beslistermijn*) enforcement.** **Partially resolved:** The decision deadline (*beslistermijn*) is calculated by a hook at the AANVRAAG stage. Awb 4:13 lid 1 designates the term set by statutory provision, so in most schemes the lex specialis carries the term (same pattern as bezwaartermijn_weken). Where no such provision exists, Awb 4:13 lid 2 states an outer bound rather than a term: the reasonable term has in any case expired when, within eight weeks of receiving the application, no decision (*besluit*) has been given nor a notification under Awb 4:14 lid 3 has been made. Eight weeks is therefore a floor that lapses as soon as the administrative body (*bestuursorgaan*) announces a new term, and a model that treats it as the deadline itself asserts something the article does not say.
   ...
   The derivation half is open. With no statutory term and no notification, the hook can compute a date that bounds lateness without knowing the term that actually applies. Whether the lifecycle should carry both a computed floor and an unknown actual term, and what the engine annotates when only the floor is passed, is unresolved.
...
6. **The choice offered by Awb 4:14 lid 3.** Within the eight weeks of Awb 4:13 lid 2 the administrative body (*bestuursorgaan*) has two lawful moves: give the decision (*besluit*), or tell the applicant, as soon as reasonably possible, within what term a decision can be expected. Taking the second move shifts the moment at which the reasonable term is exceeded, so BEHANDELING carries a branch that the current stage definition does not express. Does this need a pattern of its own, a notification (*mededeling*) stage with its own required date and announced term, or does a hook that recalculates the decision deadline (*beslistermijn*) when a mededeling_datum is present suffice? The same question applies to the extension of Awb 4:14 lid 1, where a statutory term exists and the notification must fall within it, and to Awb 4:15, where supplementation suspends the running term.
