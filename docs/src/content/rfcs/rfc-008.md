      ...
      - name: BEHANDELING
        ...
        requires:
          - name: beslistermijn_start
            type: date
        # Awb 4:13 lid 1: beslistermijn is the term set by statutory provision (lex specialis)
        #   or, absent one, a reasonable term
        # Awb 4:13 lid 2: that reasonable term has in any case expired when, within eight
        #   weeks of receiving the application, no decision has been given nor a
        #   notification under Awb 4:14 lid 3 has been made
...
stateDiagram-v2
    ...
    state "Dwangsom track<br/>(parallel)" as dwangsom_track {
    ...
    BEHANDELING --> BESLISTERMIJN_OVER: Termijn verstreken<br/>zonder besluit
...
The engine is still a pure function per stage. But the **composition** of stages into a decision (*besluit*) lifecycle is now explicit, governed by the Awb lifecycle definition, and persisted externally.
...
**Real-world fidelity.** The model handles long-running processes (asylum decisions that take months), manual steps (administrative body (*bestuursorgaan*) investigation), and asynchronous events (notification (*bekendmaking*) by post).
...
4. **Decision deadline (*beslistermijn*) enforcement.** **Partially resolved:** The decision deadline (*beslistermijn*) is calculated by a hook at the AANVRAAG stage. Awb 4:13 lid 1 designates the term set by statutory provision, so in most schemes the lex specialis carries the term (same pattern as bezwaartermijn_weken). Where no such provision exists, Awb 4:13 lid 2 states an outer bound rather than a term: the reasonable term has in any case expired when, within eight weeks of receiving the application, no decision (*besluit*) has been given nor a notification under Awb 4:14 lid 3 has been made. Eight weeks is therefore an outer bound that lapses as soon as the administrative body (*bestuursorgaan*) announces a new term, and a model that treats it as the deadline itself asserts something the article does not say.

   The enforcement half is settled. The engine does **not** enforce the deadline: if besluit_datum exceeds the decision deadline (*beslistermijn*), the engine continues normally but annotates the decision (*besluit*) with a warning. Exceeding the decision deadline (*beslistermijn*) does not invalidate the decision (*besluit*), it **expands the lifecycle** with new available paths for the interested party (*belanghebbende*): notice of default (*ingebrekestelling*, Awb 4:17), penalty payment (*dwangsom*, Awb 4:18), and appeal (*beroep*) against failure to decide in time (Awb 6:2 lid 1 sub b). These are modeled as conditional branches in the lifecycle state machine.

   The derivation half is open. With no statutory term and no notification, the hook can compute a date that bounds lateness without knowing the term that actually applies. Whether the lifecycle should carry both a computed outer bound and an unknown actual term, and what the engine annotates when only the outer bound is passed, is unresolved.

5. ~~**Withdrawal (*intrekking*) and revocation.**~~ **Resolved:** Withdrawal (*intrekking*) is a state transition in the original decision (*besluit*)'s lifecycle, not a separate lifecycle. (The Awb has no single general *intrekking*-article: subsidies are governed by Awb 4:48-4:50, other besluiten by their own grondslag or the *contrarius actus*-leer. An earlier version of this answer cited Awb 10:4-10:5, which is the *mandaat* provision and not the grondslag for *intrekking*; RFC-022 §1.2 carries the corrected reading.) An individual decision (*beschikking*) continues to exist after notification (*bekendmaking*), it can be final/irrevocable (*onherroepelijk*), withdrawn (*ingetrokken*), amended, or expired. The withdrawal (*intrekking*) itself is a nested decision (*besluit*, same pattern as question 2): it requires motivation, notification (*bekendmaking*), and can be challenged via objection (*bezwaar*). The original individual decision (*beschikking*)'s state changes as a consequence of the withdrawal (*intrekking*) decision completing its own lifecycle.

6. **The choice offered by Awb 4:14 lid 3.** Within the eight weeks of Awb 4:13 lid 2 the administrative body (*bestuursorgaan*) has two lawful moves: give the decision (*besluit*), or tell the applicant within those eight weeks that the decision cannot be given in time and name a reasonable term within which it can be expected. Taking the second move shifts the moment at which the reasonable term is exceeded, so BEHANDELING carries a branch that the current stage definition does not express. Does this need a pattern of its own, a notification (*mededeling*) stage with its own required date and announced term, or does a hook that recalculates the decision deadline (*beslistermijn*) when a mededeling_datum is present suffice? The same question applies to the notification of Awb 4:14 lid 1, where a statutory term exists: the notification must fall within that term and does not extend it, so the term is still exceeded and Awb 4:17 and 6:12 remain available. And it applies to Awb 4:15, where supplementation suspends the running term.
...
| **BESLISTERMIJN** | 4:13-4:15 | Decision must be made within the statutory term (lid 1); absent one, within a reasonable term, which has in any case expired after eight weeks unless a notification under 4:14 lid 3 has been made (lid 2). A notification under 4:14 lid 1 does not extend a statutory term; absent one, a notification under 4:14 lid 3 sets the reasonable term. Suspension possible when applicant must supplement (4:15). |
...
```mermaid
stateDiagram-v2
    [*] --> AANVRAAG_U: Aanvraag of<br/>ambtshalve initiatief
...
stateDiagram-v2
    ...
    state voorbereiding_fase {
        VOORBEREIDING --> CONSULTATIE: Internetconsultatie /<br/>advisering (optioneel)
    ...
    state "Alleen bij concretiserend BAS" as conc_bas {
        RECHTSKRACHT --> BEROEP_BAS: Direct beroep<br/>(geen bezwaar)
...
- **2026-09-25.** Awb 4:13 was read as "the decision deadline is a reasonable term, typically eight weeks". Lid 1 designates the term set by statutory provision or, absent one, a reasonable term; lid 2 states when that reasonable term has in any case expired, not the term itself. Corrected in the procedure YAML, in open question 4 (now partially resolved) and in the Awb table of Appendix A; open question 6 added on the choice Awb 4:14 lid 3 offers. The Appendix A row no longer says that a notification under 4:14 extends the term. Source: review by Mariette Lokin, [#991](https://github.com/MinBZK/regelrecht/pull/991).
