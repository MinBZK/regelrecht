...
> **Note (25 September 2026).** [RFC-044](/rfcs/rfc-044) §4 adds a `zaakkenmerk` to recorded chronolexograms in the cell runtime. It groups the grams of one application, its course and its decision within a cell's chronicle, and is not the case abstraction this RFC rejects under "The besluit as state container": it has no state and no lifecycle, and the besluit stays the state container. RFC-044 §4 also describes how that runtime treats the facts of later stages when the decision is taken, the situation the Context below calls a workaround.
