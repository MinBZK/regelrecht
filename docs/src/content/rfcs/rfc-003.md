...
1. Engine indexes all `implements` declarations at law load time [Mariette: hoe ga je om met andere formuleringen (bv. omdat de gedelegeerde regeling op meer artikelen uit de wet is gebaseerd, dus iets als 'Gelet op de artikelen 2, 3 en 4 van de Wet op de zorgtoeslag'?]
2. When executing an article with `open_terms`, the engine looks up implementations [Mariette: hoe vind je de juiste bepaling in de gedelegeerde regeling? Bij een waarde is dat nog te doen, maar bij uitwerking van andersoortige variabelen is dat vaak moeilijk]
...
6. If found: execute the implementing article to get the value [Mariette: zie ook comment bij 55: hoe vind je 'niet-numerieke' variabelen] 
...
source:
  delegation:
    ...
    article: '8' [Mariette: artikel 8, eerste lid, heeft drie onderdelen; onderdeel a gaat over verlagen bijstand, moet je dat niet specificeren?]
    select_on:
      - name: gemeente_code
        value: $gemeente_code
  output: verlaging_percentage [Mariette: het is niet per se een percentage, het is 'verlaging_bijstand' (of individuele inkomenstoeslag)]
```
...
```yaml
# New pattern: lower regulation registers itself [Mariette: strikt genomen is artikel 8 geen delegatie maar medebewind; de gemeentelijke verordening is niet lager dan de wet, hij staat ernaast] 
implements:
  - law: participatiewet
    article: '8'
...
```
[Mariette: zie opmerkingen hierboven over specificatie artikel 8 en percentage. En net als bij de implement van artikel 4 Zorgtoeslag: hoe ga je om met een Gelet op met een hele reeks artikelen?] 

The engine resolves which municipality's (*gemeente*) ordinance (*verordening*) applies; the law itself does not encode this. The engine already knows the execution scope (e.g., `gemeente_code: GM0384`) from its parameters. The `find_implementations` method uses a `matches_scope` helper that checks all scope fields on the candidate law against execution parameters. [Mariette: een potentieel probleem is dat gemeentelijke verordeningen vaak een heel andere opzet/indeling kennen dan wettelijke regelingen op rijksniveau. Hoe vind je dan de matches?] Currently supports `gemeente_code`; designed for easy extension to `provincie_code` etc. This eliminates `select_on`, `legal_basis_for`, and `source.delegation` entirely: all delegation flows through `open_terms` + `implements`.
