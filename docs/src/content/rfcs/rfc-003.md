...
Since schema v0.6.0 this is the only place an open norm is recorded, whether or not the article names who fills it. Where the law names a filler, `delegated_to` and `delegation_type` carry the name and the required regulatory layer, and the engine rejects an implementation from another layer. Where the law names nobody ("redelijkerwijs", "in bijzondere gevallen", "voor zover dat naar het oordeel van het bestuursorgaan"), both fields are absent and any competent authority may fill the term through implementing policy. The resolver already reads an absent `delegation_type` as "no requirement on the layer", so the wider reading costs it nothing.

The concept does not change with the question of whether the law appoints a filler. That question decides who may answer, and the term is open either way. Before v0.6.0 the schema had a second field, `norm_gaps`, whose `kind` distinguished `delegated`, `policy` and `open`: three ways the same open norm gets filled, recorded in a channel the engine does not read. [RFC-031](/rfcs/rfc-031) has the measurement that ended that split.

The state of the corpus is deliberately absent here. Whether the filling regulation has been harvested is no property of the law, it is not visible in the file that would have to record it, and it stops being true the next time something is harvested. That question belongs to the resolve step and the work queue of [RFC-026](/rfcs/rfc-026).
...
- Schema v0.6.0 removes the field. The corpus of enrichment round 4 bears this out: all eleven `enables` sit on an article that already carries a gap about the same delegation, so the field duplicated a record without adding one fact. From v0.3.1 to v0.6.0 it had no reader in the engine
...
A fourth pattern is deliberately absent from the table. An open norm with no filler named in the law is IoC too, with `delegated_to` and `delegation_type` left empty. It is not a separate mechanism, and it is never a marking: the language expresses the term fine and the content sits elsewhere, so no engine operation resolves it.
...
Schema v0.6.0 widened `open_terms` to carry every open norm, including the one no regulation is appointed to fill, and removed `enables` and `norm_gaps`. That is a change to the reach of the construct decided here, and RFC-031 records why.

## References

- Schema v0.4.0: `schema/v0.4.0/schema.json`
- Schema v0.6.0: `schema/v0.6.0/schema.json`, for the widened `open_terms`
