...
Since schema v0.7.0 this is the only place an open norm is recorded, whether or not the article names who fills it. Where the law names a filler, `delegated_to` and `delegation_type` record the name and the required regulatory layer, and the engine rejects an implementation from another layer. Where the law names nobody ("redelijkerwijs", "in bijzondere gevallen", "voor zover dat naar het oordeel van het bestuursorgaan"), both fields are absent and any competent authority may fill the term through implementing policy. The resolver already reads an absent `delegation_type` as "no requirement on the layer", so the wider reading costs it nothing.

The concept does not change with the question of whether the law appoints a filler. That question decides who may answer, and the term is open either way. Before v0.7.0 the schema had a second field, `norm_gaps`, whose `kind` distinguished `delegated`, `policy` and `open`: three ways the same open norm gets filled, recorded in a channel the engine does not read. [RFC-031](/rfcs/rfc-031) has the measurement that ended that split.

The state of the corpus has no place here. Whether the filling regulation has been harvested is no property of the law, it is not visible in the file that would have to record it, and it stops being true the next time something is harvested. That question belongs to the resolve step and the work queue of [RFC-026](/rfcs/rfc-026).
...
The `implements_index` is built at load time and is date-independent. It records *all* implementing relationships across all loaded versions. Temporal filtering happens at query time in `find_implementations`, which is the correct place because the calculation date is only known at execution time.
...
- Schema v0.7.0 removes the field. The corpus of enrichment round 4 bears this out: all eleven `enables` sit on an article that already has a gap about the same delegation, so the field duplicated a record without adding a fact. From v0.3.1 to v0.7.0 it had no reader in the engine
...
The table has no fourth pattern. An open norm with no filler named in the law is IoC too, with `delegated_to` and `delegation_type` left empty, so it is the same mechanism. The language expresses the term fine and the content sits elsewhere, so no engine operation resolves it and it is never a marking.
...
Schema v0.7.0 widened `open_terms` to cover every open norm, including the one no regulation is appointed to fill, and removed `enables` and `norm_gaps`. That is a change to the reach of the construct decided here, and RFC-031 records why.

## Open Questions from a Legal Review

These questions were raised in a review by Mariette Lokin ([#1093](https://github.com/MinBZK/regelrecht/pull/1093)) after the RFC was accepted. They are recorded here and not answered in the text above.

- **Several articles in one basis.** A delegated regulation often rests on more than one article of its parent law ("Gelet op de artikelen 2, 3 en 4 van de Wet op de zorgtoeslag"). `implements` takes one `law`/`article`/`open_term` per entry, and `gelet_op` is free text the engine does not read. How does the model record a regulation that implements several articles at once?
- **Finding the implementing provision.** For a numeric value the matching provision in the delegated regulation is usually easy to find. For other kinds of open terms it often is not, and municipal ordinances are frequently organized quite differently from national regulations. How does an author, or the harvester, find the provision that fills a given open term?
- **Granularity of the delegation.** Article 8(1) Participatiewet has three sub-paragraphs (*onderdelen*); sub-paragraph a concerns reducing social assistance. The examples above cite article 8 as a whole. `legal_basis` in the corpus already names "lid 1 onderdeel a", but `implements` has no lid or onderdeel.
- **Naming of the open term.** What article 8(1)(a) leaves open is the reduction of social assistance, not necessarily a percentage. (The individual income supplement is a separate matter, in onderdeel b.) `verlaging_percentage` names one possible form of it.
- **Delegation or co-government.** Strictly, article 8 Participatiewet is not delegation but co-government (*medebewind*): the municipal ordinance does not sit below the act, it sits beside it. The RFC and the schema describe it as a "lower regulation" that implements a delegated term. The position paper describes municipalities as executing national law in *medebewind* ([Rules as Executed, section 11.3](/research/rules-as-executed#sec:ownership)). Tracked in [#1620](https://github.com/MinBZK/regelrecht/issues/1620).

## References

- Schema v0.4.0: `schema/v0.4.0/schema.json`
- Schema v0.7.0: `schema/v0.7.0/schema.json`, for the widened `open_terms`
- [RFC-031: Markeringen en open normen](/rfcs/rfc-031), which makes this the single place for an open norm
