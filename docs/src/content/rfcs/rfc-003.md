...
- Schema v0.6.0 removes the field. The corpus of enrichment round 4 bears this out: all eleven `enables` sit on an article that already has a gap about the same delegation, so the field duplicated a record without adding a fact. From v0.3.1 to v0.6.0 it had no reader in the engine
...
Schema v0.6.0 widened `open_terms` to cover every open norm, including the one no regulation is appointed to fill, and removed `enables` and `norm_gaps`. That is a change to the reach of the construct decided here, and RFC-031 records why.
