...
regulatory_layers:
  ...
  - layer: MINISTERIELE_REGELING
    rank: 7
  # A provincial ordinance prevails over a municipal one: a municipal
  # provision stays in force only where it does not conflict with it
  # (Gemeentewet art. 121) and lapses where the province regulates the same
  # subject (art. 122). Waterschapswet art. 59 does the same for a water
  # board's ordinance under a provincial one. No statute ranks a water
  # board against a municipality; they are equal levels of government, and
  # rank 9 against 10 is a tie-break, not a rule of law.
  - layer: PROVINCIALE_VERORDENING
    rank: 8
  - layer: WATERSCHAPS_VERORDENING
    rank: 9
...
# If absent, a body may fill an open term only where the law makes it
# competent, for instance through policy rules for its own powers
# (Awb 4:81); no body may fill another body's open term. The engine does
# not check this yet.
delegation_rules:
  - from: WET
...
A legal review ([#1103](https://github.com/MinBZK/regelrecht/pull/1103)) raised two points. The first remains open: municipal, water board and provincial ordinances are each legislation of their own level of government. Provincial precedence follows from Gemeentewet art. 121-122 and Waterschapswet art. 59, and only where the ordinances conflict or regulate the same subject; between a water board and a municipality there is no precedence at all. Whether a linear rank is the right model for that is to be discussed in [#1621](https://github.com/MinBZK/regelrecht/issues/1621). The second point is adopted in the `delegation_rules` comment above: an administrative body is not free to fill any open term when a delegation rule is missing. The engine does not check it yet.

An organization override cannot widen what the law allows. It can narrow the national default, for instance by restricting which layers it accepts, but it cannot let a body fill an open term for which no delegation exists.
