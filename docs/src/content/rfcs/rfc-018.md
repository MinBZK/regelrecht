...
(*annotatie*, *noot*, "m.nt.") is not what this feature is called.
...
looks like (selector, motivation, body, resolution states) but is
storage-agnostic. It does not say where notes are stored, how they are loaded,
who creates them, or how they are displayed.
...
4. **Note types and storage locations**: different note types serve different
   purposes and may belong in different places.
...
**RFC-018** (`rfc-011` was free at the time of writing and has since been
claimed; `rfc-016` and `rfc-017` are claimed by PR #510). PR #328 is closed in favor of this RFC.
...
**Validation:** note files conform to `schema/v0.5.3/annotation-schema.json`,
...
```
# In a gemeente-diemen/regelrecht-diemen repo:
regulation/nl/
  gemeentelijke_verordening/
    afstemmingsverordening_participatiewet/
      2015-01-01.yaml                           # Diemen's ordinance, implements Participatiewet art. 8
annotations/
  participatiewet/
    annotations.yaml                             # Diemen's perspective on national law
  afstemmingsverordening_participatiewet_diemen/
    annotations.yaml                             # notes on their own ordinance
```

Diemen's note on the Participation Act (*Participatiewet*) might link the text
on reducing social assistance (art. 8(1)(a)) to the provision of its own
ordinance that fills it in, or add a comment explaining how Diemen applies a
provision in its municipal social assistance (*bijstand*) policy.
...
Each note has a `creator` field (part of the W3C Web Annotation Data Model)
...
| Structure or content (*structuur of inhoud*) | Notes target *text content* via TextQuoteSelector | The note on "zorgtoeslag" finds the word regardless of which article it is in. If article 2 is renumbered to article 3, the note follows the text. Article numbers appear only as recorded hints (non-authoritative, not used in resolution). |
...
Connects text to a `machine_readable` element. This is the most important type: it
...
where ambiguity state is recorded (Decision 9).
...
type covers the ambiguity-tracking use case (Decision 9).
...
files plus an RFC amendment. That is the brittleness [RFC-005](/rfcs/rfc-005) avoids for
...
A note can have both a `questioning` body (the question in prose) and a `tagging`
...
- type: Annotation
  ...
  creator: "interpreter-team"
  target:
    source: "regelrecht://wet_op_de_zorgtoeslag"   # illustrative: the quoted words are not in the Wzt
    selector:
      type: TextQuoteSelector
      exact: "beleidsregels van de Dienst Toeslagen"
  ...
  body:
    - type: TextualBody
      value: >-
        ...
        assessment but are not in the corpus. Searching Staatscourant and
        the publications of the Dienst Toeslagen.
      purpose: questioning
      format: text/plain
      language: en
    - type: TextualBody
...
The hint does not participate in resolution. An earlier revision of this RFC
specified a fast path that searched the hinted article first and returned on a
hit, but any such shortcut can hide a competing occurrence elsewhere in the
law: a stale hint after a renumbering then silently turns a genuine Ambiguous
(which the editor escalates to a human) into a confident Found on the wrong
article. Ruling out that competitor requires the full scan anyway, so a hint
cannot shorten the search without being allowed to override it — and the note
follows the *text*, article numbers are non-authoritative. The hint remains
valid in the wire format as recorded provenance.

```
resolve(selector, articles) -> MatchResult:
  1. Try exact match across all articles:
     - Search for prefix + exact + suffix as substring
     - One match: return Found (confidence 1.0)
     - Multiple matches: return Ambiguous
  2. Fuzzy match across all articles:
  ...
  3. One match (or a clear winner > 0.1 ahead): return Found, confidence = score
  4. Multiple equally-good matches: return Ambiguous
  5. No matches: return Orphaned
```
...
7. Hint does not decide: a stale hint never removes a genuine ambiguity
8. Hint fallback: outdated hint does not block the full search
...
Notes from different sources **layer**: they do not conflict. This differs from
laws, where [RFC-010](/rfcs/rfc-010) uses priority to resolve `$id`
...
2. Calls `WasmEngine.resolveNotes()` to get match positions per article
...
resolved positions, renders `<mark>` spans. Each span has:
...
> **Amended after implementation.** `AnnotatedText.vue` and its render
> composables were removed in #1139. The highlight-in-place display described
> here never earned its keep next to `ArticleText.vue`; notes are shown in the
> Notities pane instead. The resolve path above and the write path below both
> survive that removal; only the component the selection starts in changed.

#### Write path (MVP)

1. User selects text in the article view (`ArticleText.vue`, since #1139)
...
3. Validates uniqueness via `WasmEngine.resolveNote()`: if the selector
...
>
> The write runs through the **active traject**, like law and
...
> backend, so a note and a law edit made in one session share the same
> branch and PR. There is no separate per-session branch and no
> `X-Editor-Session` header: the session cookie identifies the active
...
>
> The write is **append-only**. The browser sends only
...
> style or non-LF) has an explicit destructive-shrink guard that
...
- **Two places to look.** A law's notes are in a separate directory from its YAML
...
- **Tag vocabulary is soft-validated.** A typo in a tagging value produces a
  warning, not a hard failure. That keeps the vocabulary growable without
  ceremony.
...
| `bdd/conformance/notes.feature` | New: BDD scenarios (ported + ambiguity) |
...
| `frontend/src/EditorView.vue` | Add Notities tab, use `AnnotatedText` behind a flag |
...
Raised by Mariette Lokin in [#1104](https://github.com/MinBZK/regelrecht/pull/1104):

- **Must a filling be enacted and published?** The vocabulary describes an unfilled open norm as one "whose implementing text has not been written or located". Implementing policy that fills an open norm has to be established and published in some form before it can count, which suggests "has not been enacted and published" instead (and "partially enacted and published" for `open-norm-partial`), and "Behoeft invulling" rather than "Behoeft uitleg". The same holds for the implementing policy in `needs-uitvoeringsbeleid`. This changes what the tags mean in `corpus/annotations/_vocabulary/ambiguity.yaml`, so it is a decision, not a correction. Discussed in [#1621](https://github.com/MinBZK/regelrecht/issues/1621).
- **Open norm and open term.** Is an open norm the same as an open term? The [glossary](/reference/glossary) separates them: an open term is the schema construct, an open norm the legal phenomenon.
- **Where policy rules live.** Policy rules come under many names (Besluit bestuursrecht Toeslagen, Beleidsregel bestuurlijke boeten Dienst Toeslagen), and it is not clear that every organization keeps a policy register; does the Dienst Toeslagen? How the body of rules is kept in order at every level, including where an organization brings its own rules, is an organizational question as much as a technical one.
- **A meta-norm model.** Tags could link to a meta-norm model, a top-level legal grammar that Mariette Lokin is developing with colleagues.
...
- **2026-09-25.** The federated example had Amsterdam annotate the Wet op de zorgtoeslag from a repo with its afstemmingsverordening. Amsterdam's Afstemmingsverordening Inkomensvoorzieningen was in force until 30 September 2011 ([CVDR72184](https://lokaleregelgeving.overheid.nl/CVDR72184)), and a municipality has no role in the zorgtoeslag. The example now uses Diemen's afstemmingsverordening, which is in the corpus and implements Participatiewet art. 8. The questioning example now names the policy rules of the Dienst Toeslagen, which executes the zorgtoeslag, rather than the Belastingdienst, and is marked as illustrative: the Wzt does not contain the quoted words. Source: review by Mariette Lokin, [#1104](https://github.com/MinBZK/regelrecht/pull/1104); the point that a municipality has no role in the zorgtoeslag was added when the review was worked in.
