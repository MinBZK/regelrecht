...
request holding the number in `reserved_by`.

What makes a file a placeholder is `reserved_by`, not the status word. The
content schema keys on it: a file carrying it states no `implementation` and
must be `Reserved` or `Rejected`, while every other RFC states one. A
`Reserved` file without a `reserved_by` link is a build error too, because a
reservation nobody can trace back to a branch blocks a number forever.
