...
lets the work through or hands it back with what it found. That handing back
is the point: a finding the producer never sees changes nothing, and a finding
...
flowchart TD
    ...
    SRC[Source gate<br/><i>deterministic, no model</i>] --> SRCQ{Is this the law?}
    SRCQ -->|no| QUAR[Quarantine<br/>the harvest is repaired first]
    SRCQ -->|yes| ASM

    ASM[Assemble context<br/><i>deterministic, no model</i>] --> QUAL

    QUAL[Qualify<br/>read and classify, per lid] --> QUALG{Every lid<br/>answered?}
    QUALG -->|no, and rounds left| QUAL
    QUALG -->|yes| PROD

    PROD[Produce<br/>translate within the classification] --> SCHEMA{Schema valid?}
    SCHEMA -->|no, and rounds left| PROD
    SCHEMA -->|no, rounds spent| FAIL([Fail the job])
    SCHEMA -->|yes| CHECK

    CHECK{Deterministic checks:<br/>coverage, provenance, bindings} -->|findings, and rounds left| PROD
    CHECK -->|clean, or every finding marked| ROUTE

    ROUTE{Does this article<br/>warrant challenging?} -->|no| PUB
    ROUTE -->|yes| CHAL

    CHAL[Challenge<br/>bring the translation down, per lens] --> CHALG{Any finding<br/>survives?}
    CHALG -->|no| PUB
    CHALG -->|yes| ARB

    ARB[Arbitrate<br/>judge each finding against the text] --> ARBD{Verdict}
    ARBD -->|model is wrong| PROD
    ARBD -->|the text does not decide| MARK[Record a structural choice]
    ARBD -->|finding rejected| PUB
    MARK --> PUB

    PUB([Publish]) --> DONE([Done])

    classDef det fill:#e8f0e8,stroke:#4a7a4a
    classDef agent fill:#e8eaf4,stroke:#4a5a8a
    classDef gate fill:#f6f0e4,stroke:#8a7a4a
    classDef stop fill:#f4e8e8,stroke:#8a4a4a
    class SRC,ASM,CHECK det
    class QUAL,PROD,CHAL,ARB agent
    class SRCQ,QUALG,SCHEMA,ROUTE,CHALG,ARBD gate
    class QUAR,FAIL stop
```

Green is deterministic and needs no model. Blue is an agent run in its own
process. Amber is a gate. Red ends the chain without publishing.

### What a task with a gate looks like

Every agent task has the same shape, and the shape is what makes the chain
improvable: the gate is deterministic, its findings are quotable, and the
number of rounds is bounded so a disagreement cannot spin.

```mermaid
flowchart LR
    IN([Inputs the worker mounted]) --> RUN[Agent run<br/>own process, own context]
    RUN --> OUT([Artefact on disk])
    OUT --> GATE{Gate<br/><i>deterministic</i>}
    GATE -->|passes| NEXT([Next task])
    GATE -->|findings, rounds left| FB[Feed the findings back<br/>verbatim, with what may not change]
    FB --> RUN
