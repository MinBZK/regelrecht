...
*Superseded on this point by [RFC-036](/rfcs/rfc-036).* The two paragraphs
above were written when `null` did double duty as "unknown". RFC-036 splits
the two: an **unknown** value (a fact nobody has yet) propagates exactly as
described here, as the union of the missing facts, and a definite `true` or
`false` still settles `OR` and `AND`. A **null** (an absence the data is
authoritative about) is a value: a `filter` or a `body` that evaluates to
`null` under `ADD`, `MIN`, `MAX`, `AND` or `OR` is an error the law has to
resolve by testing for the absence first. A `null` collection still iterates
nothing, and `MIN`/`MAX` over an empty collection still return `null`; summing
that `null` one level up is then an error, not an unknown.
...
**In the docs:** [Collections](/concepts/collections).
