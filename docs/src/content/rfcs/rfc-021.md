...
Laws reason about dates relative to a peildatum (the reference date a law is evaluated as of). A deadline expires a number of weeks after a decision. Information older than five years triggers a heavier duty to motivate. A request counts as timely when it arrives on or before a cutoff. Authors writing machine-readable logic for these rules run into two limitations.
...
The engine already provides most of what is needed: four date operations (`AGE`, `DATE_ADD`, `DATE`, `DAY_OF_WEEK`), a date parser that accepts ISO 8601 strings and reference-date objects, and the peildatum injected as `$referencedate`. The gap is narrow and additive.

## Decision

Add date support along two paths.
...
No new operator names. `$peildatum > $grensdatum` works, the same way `$bedrag > 1000` does. `EQUALS` is made date-aware for one pair of forms: a date string against the reference-date object, so `"2025-01-01"` equals `{iso: "2025-01-01", ...}`. Two strings or two objects keep structural equality, which for canonical date strings already coincides with chronological equality. Combined with the canonical-form requirement on date strings (see Implementation Notes), `EQUALS` and the ordering operators can never disagree on the same pair of dates.
...
That reservation is what [RFC-032](/rfcs/rfc-032) later drew on. It rejected `YEAR`, `MONTH`, `DAY_OF_MONTH` and `START_OF_MONTH` as four separate names and settled on `DATE_PART` and `START_OF`, each with the same `in` argument, on the rule that a parameter is allowed when it ranges over a closed dimension the language already holds. For `DATE_PART` that dimension is the argument list of `DATE`, of which it is the inverse. `DAY_OF_WEEK` stays as it is, because a weekday falls outside that decomposition.
