...
The current schema version is **v0.7.1**.
...
|---------|-----------|-----|
| v0.7.1 | Descriptions only: six field descriptions said the Rust law-model drops or cannot read a field it does read (`endpoint`, `requires`, `references`, `resolve` and `legal_basis` on an action, and the preamble's `machine_readable`); they now say what the model and the engine do with it. No keyword that affects validation changed, so a file valid under v0.7.0 is valid under v0.7.1 unchanged | [RFC-040](/rfcs/rfc-040) |
