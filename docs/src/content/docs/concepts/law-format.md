...
| **Rounding** | `ROUND`, `CEIL`, `FLOOR`, `TRUNCATE` | `value:` + `precision:` (a single operand, rounded to N decimals; see below) |
...
Rounding is an **explicit, law-modeled instruction** — the engine never rounds a value implicitly (not even money). A law that must round says so with one of four unary operations, each taking a single `value:` operand and a `precision:` (the number of decimal places to round to, in the value's own [unit](#type-specifications)):
...
| `FLOOR` | down (toward −∞) | *naar beneden afronden* |
| `TRUNCATE` | toward zero | *afkappen* |
