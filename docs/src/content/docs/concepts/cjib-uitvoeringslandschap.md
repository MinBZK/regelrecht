...
8. **Bezwaar-routing**. Each CJIB-emitted FCID event carries a `bezwaar_route` derived at emission time from the decretogram's RFC-008 procedure state. The route is not a per-rule field; it is computed at the BEKENDMAKING stage from the AWB-6:7/6:8 hooks plus the cell's bezwaar-intake configuration (see [RFC-022 §3.3](/rfcs/rfc-022) and the [MBO/FCID integration document](/integrations/mbo-fcid)). For decretograms that CJIB itself produces (Wahv-sanctie), the bezwaar-route points to CJIB's eigen bezwaar-intake. For decretograms that CJIB carries on behalf of another cell (a CAK-besluit, an OM-strafbeschikking), the route points to that other cell. The mechanics of this in the wettelijke routing per regulation need validation per case.
