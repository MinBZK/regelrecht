...
Staat de regelgeving na verloop van tijd ergens anders in de repo — bijvoorbeeld omdat er werkmappen naast het corpus zijn gekomen — dan past een beheerder het subpath aan onder **Instellingen → Algemeen → Subpath**. Leeg laten betekent repo-root.
...
Alleen een beheerder van het traject kan dit, en alleen op een eigen repo: trajecten die naar het centrale corpus schrijven houden het vaste pad `regulation/nl`.
