...
- **Harvester-admin proxy**: `/api/harvest-admin/{*rest}`, a single wildcard route accepting any method. It rewrites `/api/harvest-admin/<x>` to `/api/<x>` on the standalone harvester-admin service and forwards the session cookie. This is how the editor's Harvester section reaches that API; the browser never calls it directly. The route is gated on `harvester-reader` as defence in depth, and harvester-admin remains the real enforcer for writer and admin actions.
