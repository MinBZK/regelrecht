      ...
      - parameter: <array parameter>
        ...
        bronnen:
          - cel: <id>
            ...
            invoer: {<input>: {kolom: <column of the row>}}   # or lexostatus/veld, parameter, regeling/uitkomst, waarde
...
Every parameter of the decision comes from exactly one source. The lexostatuses of the case reduce the case in the cell: the application, and the course of the case, such as a request to supplement or a suspension of the decision period. Their provenance is `eigen`: the process's own cell. The other synthesis sources supply facts of other cells, with their input taken from one of those lexostatuses. The decision form holds the judgments of the case handler, which only exist at the moment of deciding; they go in with provenance `behandelaar`. The form is not configured: it is every parameter of the decision with origin `OORDEEL` (see "Who supplies a parameter"), in the order the article declares them, labelled with the parameter's description (or the part after "Naam:" when there is one, else its name) and grouped by the article of its `grondslag`. The state at decision covers facts that can only arise later, such as the notification of the decision. It is not configured either: the law carries it. The procedure of the decision's legal character (RFC-008, `procedure` with stages and `requires`, in the Awb) names in a later stage what only arises then, for example the notification facts in stage `BEKENDMAKING`. The event in which the decision is recorded has a stage (`BESLUIT`). Every parameter of the decision that a stage after that one requires has not happened at the moment of deciding: a boolean is `false`, anything else `null`, with provenance `stand_bij_besluit` and the stage in which it arises. The runtime derives this at load time; a recording event whose stage the procedure does not know is a start-up error. Without a procedure there is no state at decision, and the provenance check reports what is missing.
...
| `REGISTER` | a synthesis source that supplies the parameter (under the name the process's synthesis gives it), and whose chronicle has a stream with a `grondslag` in the regulation of `register`; or synthesis per row from a table such a source passes on. A source with a url cannot be inspected and counts. |
...
A source speaks the language of its own regulation. A register cell names a fact as the register regulation names it, per article (`is_ingeschreven_in_register`); the consumer asks for it under the name of its own article (`is_ingeschreven_gemeenteraad`). The translation belongs to the consumer: `parameters` as a map from the name at the source to the consumer's parameter. The source may supply that name as a parameter or as an extra field: whether a fact is a parameter is up to the consumer's law, not the source. A fixed input (`waarde`) lets one lexostatus serve several questions, such as the register of each body.
...
| `POST /cellen/<id>/api/lexostatus/{naam}/proef` | Runtime token only (401 without, 403 with another). `{concept, inputs}` to `{gram, lexostatus}`. Builds the gram of the draft in memory and reduces the chronicle with that gram added. Without an input `zaakkenmerk`, the draft's case identifier counts. Records nothing. |
...
| `GET /processen/<id>/api/mogelijkheden` | Portal only. What `portaal.aanbod` offers the logged-in person, per period the policy offers (`aanbod.tijdvakken`), with the deadline and the trace of the run. Each possibility names its period as `tijdvak: {parameter, veld, waarde}`. Records nothing. |
...
36. **The periods on offer are policy.** Which values the portal offers for the period is an outcome of the actor's policy (`aanbod.tijdvakken`), computed on today's date. A period is still a year; other granularities are for a later step.
