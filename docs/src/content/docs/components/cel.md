...
The cell finds its decision in the law. Without `regeling`, the runtime looks at load time for the article that produces a `BESCHIKKING` and whose competent authority (of the article, else of the regulation) equals the cell's `recording_actor`, compared after normalizing case and punctuation. Exactly one such article is the decision, and every listed outcome must come from it; none or several is a start-up error that names the candidates, and then `regeling` settles it. Which outcomes make up the decision stays configuration.
...
Nobody lists which applications an organisation can make, not in the law and not in the cell. The actor decides what each portal offers, and on what grounds, in a policy regulation of its own. `portaal.aanbod` names one outcome of that regulation, and optionally a second outcome of the same article that gives the deadline. `GET /api/mogelijkheden` builds an empty draft for this year and the next (only the subsidy year), adds what the login supplies, runs the synthesis, and evaluates the regulation in one run per year.

The offer outcome gets a verdict. A definite zero or false means the policy offers nothing (`uitgesloten`). An unknown outcome whose missing facts are all facts the application supplies means an application is possible: those facts are what the application has to bring (Awb 4:2 paragraph 2). A definite positive value is possible as well. The verdict looks only at what the offer outcome itself lacks. A fact the deadline needs does not count, and neither does one another outcome of the same run needs. It is `niet_te_bepalen` when the outcome is empty, when the engine fails without naming a missing fact, or when a missing fact is one a source cell should have supplied, since that fact will not come from the application.
...
The frontend shows one button per subsidy year to start an application. It is disabled unless the verdict is possible. A question mark next to it opens the verdict, the reason, the deadline and the trace of the run.
...
6. The `portaal` block names an existing event, a lexostatus that picks a gram, and a regulation outcome whose article is in the `grondslag` of the event. An offer names an outcome that exists, and a deadline outcome from the same article, because the two are evaluated in one run.
7. Synthesis needs a portal or a decision. Each input comes from a field of the check's lexostatus, or from an extra field an earlier source passes on; that earlier source takes its own inputs from the check's lexostatus. A source supplies a parameter or passes an extra field. Each parameter is a parameter of the article of the check, the decision or the offer, or of an article one of them calls, transitively through `source`. A parameter comes from exactly one place: the own reduction or one source.
...
The `portaal` block in `cel.yaml` says which event a submission becomes, which lexostatus and outcome the check before submitting evaluates (`toets: {lexostatus, regeling, uitkomst}`), optionally what the portal offers (`aanbod: {regeling, uitkomst, termijn}`, with `termijn` optional), and optionally which form file supplies labels and order (`formulier: {pad, scherm}`). The form file never changes behavior: a field it does not know gets its field name as label, and a field the stream does not know is skipped.
...
| `POST /cellen/<id>/api/eherkenning/login` | Portal only. `{kvk, persoon}` to a session. A KvK number has eight digits and the name is not empty. The login checks nothing against a register: whether the person may act for the organisation is for the policy regulation to decide, through synthesis with a register cell. |
...
| `GET /cellen/<id>/api/mogelijkheden` | Portal only. What `portaal.aanbod` offers the logged-in person, per subsidy year, with the deadline and the trace of the run. Records nothing. |
...
27. **Deriving what can be applied for.** The position paper and RFC-022 say nothing about which acts an actor may perform, or about applications at all. The cell does not list them either: a policy regulation of the actor states what each portal offers and on what grounds, and the portal runs that regulation on an empty draft. This is informing by the actor, in the paper's sense, and records nothing.
