...
A cell has no login. Its reading routes (chronicle, cases, lexostatuses, streams) are open to any consumer, the same way a register of another organization would be (deviation 10). Recording a gram and a trial reduction are reserved for the processes of the runtime itself: at every start the runtime makes a random token, the internal transport sends it in the `x-cel-runtime-token` header, and the cell answers 401 without it and 403 with another one (deviation 37). `PROCESSES_PATH` is optional: without it, the runtime serves only cells.
...
4. **The gram.** What the cell writes, one JSON line per gram in `DATA_DIR/<cel>/<chronicle>.jsonl`. A gram is appended and never changed; a correction is a new gram. It carries `kind`, `type`, `soort`, `stage` (for a stage decretogram), `name`, `chronicle`, `recording_actor`, `grondslag`, `op_moment` (when the fact legally holds or happened), `vastgelegd_op` (when the cell recorded it), `stroom {id, sha256}` and `fields`, and `herkomst: startstand` when it was placed at start-up. When the event bound `op_moment` to a submitted value, the gram also carries `op_moment_grondslag`. A gram of a decision the cell took itself also carries `legal_character`, `decision_type`, `regulation`, `regulation_valid_from`, `competent_authority` when the regulation names one, `inputs` and `receipt` (see "Taking the decision"). A gram of an event with a case also carries `zaak` and `zaakkenmerk`; `gram.json` requires the `zaakkenmerk` then and forbids it otherwise. `op_moment` and `vastgelegd_op` must be moments with a time zone. A line written before `vastgelegd_op` existed still loads: the runtime reads it as its `op_moment`, with a warning. The file is the source; the runtime also keeps the grams in memory with an index per case, so a reduction or a case query does not read the file again, and nobody but the runtime writes to it. A line counts only once it ends with a newline: an incomplete last line (the runtime stopped while writing) is cut off at start-up with a warning, and an unreadable line before it stops the runtime from starting (nothing is cut off then). Every write starts at the length the cell knows, so the remains of an earlier failed write are overwritten. Readers do not wait for the disk; only writing waits for fsync.
...
A parameter can say who supplies it, per the law, with `origin` and always a `grondslag` (RFC-043): `BELANGHEBBENDE` (what the applicant supplies or chooses), `DOSSIER` (a fact from the course of the case), `OORDEEL` (a judgment given when deciding), `REGISTER` with `register: <regulation>` (a fact from a register kept under that regulation) and `KANAAL` (what the login says). The period of the decision requested carries `rol: TIJDVAK`, with `BELANGHEBBENDE`. An article of an implementing policy (`UITVOERINGSBELEID`) can override the origin of a parameter of another regulation with `origins` in `machine_readable`. For a process, the override counts when the competent authority of that policy is the process's actor; otherwise the law's origin applies. Two overrides that disagree are a start-up error.
...
| `OORDEEL` | the decision form, for the decision only; nothing else may supply it |
| `REGISTER` | a synthesis source that lists the parameter, whose lexostatus names an article asking for it in `levert_aan`, and whose chronicle has a stream with a `grondslag` in the regulation of `register`; or synthesis per row from a table such a source passes on |
...
- `op_moment`: when the fact legally holds or happened. By default the moment of recording. An event can bind it to a submitted value with a legal ground, and the gram then carries that ground as `op_moment_grondslag`. The value is a date (the start of that day) or a moment with a time zone, and never later than the recording: what still has to happen is not a fact. Bind it to `$intake` when whoever submits the fact may not choose the moment. An application that arrives by another route (on paper) is the example: the counter gives the day of the date stamp, the receipt counts in law (Awb 4:1, 4:13: the decision period runs from receipt), and the portal never supplies the value, so an applicant cannot backdate.
- `vastgelegd_op`: when the cell recorded the gram, always the cell's own clock; for a start state the load time.
...
The process passes a peil wherever the moment matters. A (trial) decision reads every cell at the peildatum on which the engine reads the regulation, so a fact that only takes effect after the decision does not count, and the same trial decision gives the same state later. The check before submitting reads the sources as of today. The offer for a period that has yet to start reads them as of its first day (1 January of the subsidy year), so a strike-off effective before that period counts, instead of the registers of today standing in for a year that is still to come. The period is a year here (deviation 36); moving the reference date of Wpp art. 106 into the law is a separate step, and can use the same mechanism.
...
7. Once synthesis and decision pass: every parameter of the check, the offer and each outcome of the decision has a supplier that fits its applicable origin and none that does not, and the overrides in implementing policy do not conflict (see "Who supplies a parameter"). Before that, loading the corpus checks the form of every `origin` and `origins` entry.
...
| `GET /cellen/<id>/api/lexostatus/{naam}?<input>=...` | A reduction, with the inputs as query parameters. No login. Optional `peilmoment` and `bekend_op` (a date or a moment) read the chronicle at an earlier moment (see "Time"); 400 when either is not a date or moment. |
...
| `POST /processen/<id>/api/aanvraag/toets` | Portal only. Has the cell reduce the draft on trial, runs the synthesis (the sources read as of today) and evaluates the configured outcome. `ontbreekt` lists the parameters of a presence derivation (`gevuld`, `tabel` with `elke_regel`) that came out false. |
| `POST /processen/<id>/api/aanvraag` | Portal only. Has the cell record the gram and returns it |
| `GET /processen/<id>/api/mogelijkheden` | Portal only. What `portaal.aanbod` offers the logged-in person, per period from `aanbod.keuzes`, with the deadline and the trace of the run. Each possibility names its period as `tijdvak: {parameter, veld, waarde}`, and the `peilmoment` at which the sources were read (see "Time"). Records nothing. |
