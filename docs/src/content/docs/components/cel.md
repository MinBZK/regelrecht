...
The offer tests only conditions that are settled beforehand: who logs in, what the registers know, and the subsidy year. Whether all the facts an application needs will be there cannot be known before the form is filled in; that is the check after filling it in (completeness) and then the decision. So the verdict is simple. True (or a positive value) means an application is possible (`mogelijk`). A definite zero or false means the policy offers nothing (`uitgesloten`). Everything else is `niet_te_bepalen`: an empty outcome, a missing fact, an engine error. The verdict looks only at the offer outcome itself; a fact the deadline needs does not count.

The runtime refuses to start when the offer could lean on something that is not settled beforehand. Every parameter the article of the offer outcome asks of its caller must come from the login (a derivation of the check lexostatus that reads only `$intake` fields), from a synthesis source, or be `subsidiejaar`. Otherwise the start-up check says "aanbod: voorwaarde leunt op '<parameter>', dat vooraf niet bekend is". The check works per article rather than per outcome, because the engine evaluates the whole article for any outcome of it.
...
3. The `portaal` block names an existing event of that cell that binds only `$intake` paths the portal supplies, a lexostatus that reads that event and picks a gram (not a list, with `zaakkenmerk` as its only input), and a regulation outcome whose article is in the `grondslag` of the event. An offer names an outcome that exists, and a deadline outcome from the same article, because the two are evaluated in one run. Every parameter the article of the offer asks for comes from the login, a synthesis source, or is `subsidiejaar` (see "What an organisation can apply for").
