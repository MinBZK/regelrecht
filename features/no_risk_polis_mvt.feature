Feature: No-risk polis — scenarios uit Memorie van Toelichting
  ...
  Scenario: Werknemer banenafspraak met LKS heeft recht op no-risk polis (MvT § 2.1)
    ...
    When the law "ziektewet" is executed for outputs "heeft_recht_op_no_risk_polis,voldoet_aan_lid_2"
    Then the execution succeeds
    And the output "heeft_recht_op_no_risk_polis" is "true"
    And the output "voldoet_aan_lid_2" is "true"
  ...
  Scenario: Lid 4 — voortzetting dienstbetrekking na WIA-vaststelling geeft 5 jaar recht
    ...
    When the law "ziektewet" is executed for outputs "heeft_recht_op_no_risk_polis,voldoet_aan_lid_4,duur_no_risk_polis_jaren"
    Then the execution succeeds
    And the output "heeft_recht_op_no_risk_polis" is "true"
    And the output "voldoet_aan_lid_4" is "true"
