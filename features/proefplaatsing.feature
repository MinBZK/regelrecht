Feature: Proefplaatsing (Werkloosheidswet artikel 76a)
  ...
  Scenario: WW-gerechtigde met aansprakelijkheidsverzekering en uitzicht op vaste baan mag proefplaatsing aangaan
    ...
    When the law "werkloosheidswet" is executed for outputs "mag_proefplaatsing_aangaan,max_duur_proefplaatsing_maanden,ww_uitkering_blijft_bestaan,voldoet_aan_lid_3_voorwaarden"
    Then the execution succeeds
    And the output "mag_proefplaatsing_aangaan" is "true"
    And the output "voldoet_aan_lid_3_voorwaarden" is "true"
    And the output "max_duur_proefplaatsing_maanden" is "6"
    And the output "ww_uitkering_blijft_bestaan" is "true"
  ...
  Scenario: WW-gerechtigde zonder aansprakelijkheidsverzekering mag geen proefplaatsing aangaan
    Given a citizen with the following data:
      ...
      | niet_eerder_proefplaatsing_zelfde_werkgever    | true      |
      | reeel_uitzicht_op_dienstbetrekking_zes_maanden | true      |
    When the law "werkloosheidswet" is executed for outputs "mag_proefplaatsing_aangaan,voldoet_aan_lid_3_voorwaarden"
    Then the execution succeeds
    And the output "mag_proefplaatsing_aangaan" is "false"
    And the output "voldoet_aan_lid_3_voorwaarden" is "false"
  ...
  Scenario: Eerdere proefplaatsing bij zelfde werkgever blokkeert nieuwe proefplaatsing
    Given a citizen with the following data:
      ...
      | reeel_uitzicht_op_dienstbetrekking_zes_maanden | true      |
    When the law "werkloosheidswet" is executed for outputs "mag_proefplaatsing_aangaan"
    Then the execution succeeds
    And the output "mag_proefplaatsing_aangaan" is "false"
