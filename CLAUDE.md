...
- `frontend-chrono-poc/` - De frontend van de chronolexografie-testopstelling (Vue/Vite op de wereld-API van `packages/simulator`): een kolom per cel met haar kronieken, de grammen gekleurd per soort, per waarde in een decretogram de herkomst, een tijdlijn met de klok en "spoel vooruit tot", de acties uit het wereldbestand, de instellingen en het observatielog als gelabeld meetinstrument. Casus-onwetend: elke naam op het scherm komt uit het beeld dat de server geeft
