...
- `packages/admin/` - Harvester-admin API (Rust; standalone harvest job API). Its dashboard UI now lives in the editor as the "Corpusinwinning" section (`frontend/src/harvester/`), reached through the editor-api `/api/harvest-admin/*` proxy; the API stays independently addressable for scripts/services.
