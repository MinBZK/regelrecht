...
Controles na afloop: `validate` over beide corpora 114 bestanden OK (0 bevindingen); `yamllint` schoon; `just bdd` 137 scenario's / 1119 stappen groen; `just bdd-demo` 331 scenario's / 2664 stappen groen; `frontend-demo` vitest 11 bestanden / 97 tests groen; `just wasm-build` slaagt.
...
### Gewijzigde scenario-verwachtingen
...
Alles wat hierboven niet staat, blijft open en staat in de per-wet secties van dit rapport en in de tabel per categorie. In het bijzonder:

- De volledige categorie **scenario** (60 punten): modellering gebogen naar POC- of scenariodata en weggelaten leden zonder untranslatable (Anw AO-drempel, Bbz-urencriterium, Awb 7:1-gronden, Kieswet Caribisch gebied, en de rest). Die vragen per wet een tekstlezing en horen niet in een PR die alleen aantoonbare afwijkingen rechttrekt.
- De categorie **poc** op één punt na: de verouderde FOREACH-untranslatable in dertien bestanden, `voldoet_aan_voorwaarden` over alle registraties bij curator en wsnp, de AOW-formule en kortingsdeler, `default: 0.38`, dode definities, de kapstok op het vervallen art. 2.11.
- De categorie **data** buiten terrassen, levensgedrag, `partner_buitenlands_inkomen` en de lege strings: dubbele box-3-vrijstelling, ALO-kop 3.480, zorgtoeslagpercentages 2024, AIO art. 21/22.
- De categorieën **engine**, **checker** en **onbekend** integraal (default-tak als vangnet bij curator CITIZEN, WGBO-rangorde, `type_spec 65..70`, grondslagverwijzingen Archiefwet en LAA-drempel), en de misleidende naam `drempelinkomen_alleenstaande` in zorgtoeslagwet 2025 (rekenkundig correct, verkeerd benoemd, vermogenstoets bij art. 2 in plaats van 2a).
- Alle negen punten onder "Wat de taal nog niet kan (RFC-kandidaten)". Twee daarvan raken deze PR direct: de delegatieregisters en SUWI toetsen `datum_einde`/`end_date` nog steeds op null zonder dat het veld nullable kan worden gedeclareerd (punt 1), en de lege-string-normalisatie in JSON-arraycellen van scenario-tabellen (punt 9) is hier met de hand gedaan in plaats van door `apply_absent_semantics.mjs`.
